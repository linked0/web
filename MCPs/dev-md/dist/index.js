...
const DEV_MD_PATH = path.join(process.env.HOME || "", "work", "web", "dev.md");
...
class DevMdServer {
    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
            tools: [
                ...
                {
                    name: "search_docs",
                    description: "Search across all development documentation (dev.md and zsub folder) for specific keywords or topics",
                    inputSchema: {
                        type: "object",
                        properties: {
                            query: {
                ...
                },
                {
                    ...
                    description: "Get configuration and setup instructions for a specific project (betelgeuse, poohnet, tigger-swap)",
                    inputSchema: {
                        type: "object",
                        properties: {
                            project_name: {
                ...
                },
                {
                    ...
                    description: "Get environment setup guide for Mac development environment",
                    inputSchema: {
                        type: "object",
                        properties: {
                            category: {
                                type: "string",
                                description: "Specific category (tools, config, shortcuts, vscode, hardhat, foundry, etc.) or 'all'",
                                default: "all",
                            },
                        },
                    },
                ...
                },
                {
                    ...
                    description: "Search error.md for solutions to common development errors",
                    inputSchema: {
                        type: "object",
                        properties: {
                            error_keyword: {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                ...
                switch (name) {
                    case "soonguri":
                        return await this.soonguri(args.query, args.context_lines || 3);
                    case "search_docs":
                        return await this.searchDocs(args.query, args.context_lines || 3);
                    case "get_project_config":
                        return await this.getProjectConfig(args.project_name);
                    case "get_setup_guide":
                        return await this.getSetupGuide(args.category || "all");
                    case "list_projects":
                        return await this.listProjects();
                    case "get_error_solutions":
                        return await this.getErrorSolutions(args.error_keyword);
                    default:
    ...
    async soonguri(query, contextLines) {
        return {
            content: [
                {
                    type: "text",
    ...
    async searchDocs(query, contextLines) {
        const results = [];
    ...
    async getSetupGuide(category) {
        const workEnvContent = await this.readFile(path.join(ZSUB_PATH, "work-env.md"));
        if (category === "all") {
            return {
                content: [
                    {
    ...
    async getErrorSolutions(errorKeyword) {
        try {
