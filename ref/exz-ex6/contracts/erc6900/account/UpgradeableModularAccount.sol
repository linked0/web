  ...
  UUPSUpgradeable
{
  ...
  function initialize(
    ...
    if (
      length != manifestHashes.length || length != pluginInstallDatas.length
    ) {
  ...
  function executeFromPluginExternal(
    ...
    .permittedExternalCalls[IPlugin(msg.sender)][target].addressPermitted &&
      ...
      .permittedExternalCalls[IPlugin(msg.sender)][target]
        ...
        _storage
        .permittedExternalCalls[IPlugin(msg.sender)][target].permittedSelectors[
            selector
          ]);
  ...
  function _doUserOpValidation(
    ...
    EnumerableMap.Bytes32ToUintMap
    ...
    for (uint256 i = 0; i < preUserOpValidationHooksLength; ) {
      ...
      if (!preUserOpValidationHook.isEmptyOrMagicValue()) {
        ...
        currentValidationData = IPlugin(plugin).preUserOpValidationHook(
          functionId,
          userOp,
          userOpHash
        );
    ...
    // Run the user op validationFunction
    {
      if (!userOpValidationFunction.isEmptyOrMagicValue()) {
        ...
        currentValidationData = IPlugin(plugin).userOpValidationFunction(
          functionId,
          userOp,
          userOpHash
        );
  ...
  function _doRuntimeValidationIfNotFromEP() internal {
    ...
    // Identifier scope limiting
    {
      if (!runtimeValidationFunction.isEmptyOrMagicValue()) {
        ...
        {} catch (bytes memory revertReason) {
          revert RuntimeValidationFunctionReverted(
            plugin,
            functionId,
            revertReason
          );
  ...
  function _runPreExecHook(
    ...
    try
      IPlugin(plugin).preExecutionHook(functionId, msg.sender, msg.value, data)
