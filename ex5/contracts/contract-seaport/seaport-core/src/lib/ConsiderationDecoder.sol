...
import {AdvancedOrder, ConsiderationItem, CriteriaResolver, Fulfillment, FulfillmentComponent, OfferItem, Order, OrderParameters, ReceivedItem} from "../../../seaport-types/src/lib/ConsiderationStructs.sol";

import {AdvancedOrder_denominator_offset, AdvancedOrder_extraData_offset, AdvancedOrder_fixed_segment_0, AdvancedOrder_head_size, AdvancedOrder_numerator_offset, AdvancedOrder_signature_offset, AdvancedOrderPlusOrderParameters_head_size, Common_amount_offset, Common_endAmount_offset, Common_identifier_offset, Common_token_offset, ConsiderationItem_recipient_offset, ConsiderationItem_size_with_head_pointer, ConsiderationItem_size, CriteriaResolver_criteriaProof_offset, CriteriaResolver_fixed_segment_0, CriteriaResolver_head_size, ThreeWords, FreeMemoryPointerSlot, Fulfillment_considerationComponents_offset, Fulfillment_head_size, FulfillmentComponent_mem_tail_size_shift, FulfillmentComponent_mem_tail_size, generateOrder_maximum_returned_array_length, OfferItem_size_with_head_pointer, OfferItem_size, OneWord, OneWordShift, OnlyFullWordMask, Order_head_size, Order_signature_offset, OrderComponents_OrderParameters_common_head_size, OrderParameters_consideration_head_offset, OrderParameters_head_size, OrderParameters_offer_head_offset, OrderParameters_totalOriginalConsiderationItems_offset, ReceivedItem_recipient_offset, ReceivedItem_size, ReceivedItem_size_excluding_recipient, SpentItem_size_shift, SpentItem_size, ThirtyOneBytes, TwoWords} from "../../../seaport-types/src/lib/ConsiderationConstants.sol";
...
contract ConsiderationDecoder {
    ...
    )
  {
    assembly {
      ...
      if iszero(invalidEncoding) {
        ...
        consideration, invalidReceivedItems := copyReceivedItemsAsConsiderationItems(
      ...
      function copySpentItemsAsOfferItems(
        ...
        for {
          ...
          mstore(add(mPtrTailNext, Common_endAmount_offset), newAmount)
      ...
      function copyReceivedItemsAsConsiderationItems(
        ...
        mstore(
          ...
          add(
            ...
            add(
              OneWord,
              mul(newLength, ConsiderationItem_size_with_head_pointer)
            )
          )
        )
        ...
        let headSizeToCompareWithLength := shl(OneWordShift, add(1, newLength))
          ...
          invalidReceivedItems := or(
            ...
            or(
              ...
              or(
                ...
                checkRecipients(
                  mload(
                    add(mPtrTailOriginalNext, ReceivedItem_recipient_offset)
                  ),
      ...
      function checkRecipients(originalRecipient, newRecipient) -> isInvalid {
        isInvalid := iszero(
          or(iszero(originalRecipient), eq(newRecipient, originalRecipient))
        )
      }
  ...
  function _toOfferItemInput(
    function(ReceivedItem memory, address, bytes32, bytes memory) internal inFn
  )
    internal
    pure
    returns (
