...
import {AdvancedOrder, ConsiderationItem, CriteriaResolver, Execution, Fulfillment, FulfillmentComponent, OfferItem, OrderParameters, ReceivedItem} from "../../../seaport-types/src/lib/ConsiderationStructs.sol";

import {OrderFulfiller} from "./OrderFulfiller.sol";

import {FulfillmentApplier} from "./FulfillmentApplier.sol";

import {_revertConsiderationNotMet, _revertInvalidNativeOfferItem, _revertNoSpecifiedOrdersAvailable} from "../../../seaport-types/src/lib/ConsiderationErrors.sol";

import {Error_selector_offset, InsufficientNativeTokensSupplied_error_selector, InsufficientNativeTokensSupplied_error_length} from "../../../seaport-types/src/lib/ConsiderationErrorConstants.sol";

import {AccumulatorDisarmed, ConsiderationItem_recipient_offset, Execution_offerer_offset, NonMatchSelector_InvalidErrorValue, NonMatchSelector_MagicMask, OneWord, OneWordShift, OrdersMatchedTopic0, ReceivedItem_amount_offset, ReceivedItem_recipient_offset, TwoWords} from "../../../seaport-types/src/lib/ConsiderationConstants.sol";
...
contract OrderCombiner is OrderFulfiller, FulfillmentApplier {
  ...
  function _validateOrdersAndPrepareToFulfill(
...

    {
      ...
      if (
        invalidNativeOfferItemErrorBuffer == NonMatchSelector_InvalidErrorValue
      ) {
        ...
        if (advancedOrder.parameters.orderType != OrderType.CONTRACT) {
          ...
          if (
            !_updateStatus(
              ...
              _revertOnFailedUpdate(advancedOrder.parameters, revertOnInvalid)
            )
          ) {
  ...
  function _performFinalChecksAndExecuteOrders(
    ...
    unchecked {
      // Determine the memory offset to terminate on during loops.
      uint256 terminalMemoryOffset = ((executions.length + 1) << OneWordShift);
          ...
          for (uint256 j = 0; j < totalConsiderationItems; ++j) {
            ...
            assembly {
              ...
              mstore(
                ...
                mload(
                  add(considerationItem, ConsiderationItem_recipient_offset)
                )
              )
            }
          }
