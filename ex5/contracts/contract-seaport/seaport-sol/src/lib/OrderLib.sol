...
import {AdditionalRecipient, AdvancedOrder, BasicOrderParameters, ConsiderationItem, OfferItem, Order, OrderParameters, OrderType} from "../../../seaport-types/src/lib/ConsiderationStructs.sol";
...
library OrderLib {
  ...
  function copy(Order memory item) internal pure returns (Order memory) {
    return
      Order({parameters: item.parameters.copy(), signature: item.signature});
  }
