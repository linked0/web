...
import {CriteriaResolver, OfferItem, OrderComponents, AdvancedOrder} from "seaport-types/src/lib/ConsiderationStructs.sol";
...
contract FulfillAdvancedOrderCriteria is BaseOrderTest {
  ...
  function testFulfillAdvancedOrderWithCriteria(FuzzArgs memory args) public {
    test(this.fulfillAdvancedOrderWithCriteria, Context(consideration, args));
    test(
    ...
    )
  {
    ...
    for (uint256 i = 0; i < context.args.identifiers.length; i++) {
      ...
      hashedIdentifiers[i] = keccak256(abi.encode(context.args.identifiers[i]));
    }
