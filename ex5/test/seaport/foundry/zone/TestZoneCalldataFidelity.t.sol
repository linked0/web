...
import {AdvancedOrder, ConsiderationItem, CriteriaResolver, ItemType, OfferItem, OrderParameters, ZoneParameters} from "seaport-types/src/lib/ConsiderationStructs.sol";
...
contract PreAndPostFulfillmentCheckTest is BaseOrderTest {
  ...
  function testCalldataEquivalence(TestCase memory testCase) public {
    ...
    test(
      this.execCalldataEquivalence,
      Context({consideration: referenceConsideration, testCase: testCase})
    );
  }
  ...
  function execCalldataEquivalence(Context memory context) public stateless {
    ...
    bytes32 expectedZoneHash = bytes32(keccak256(abi.encode(zoneParameters)));
