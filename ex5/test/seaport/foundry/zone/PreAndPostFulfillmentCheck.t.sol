...
import {TestTransferValidationZoneOfferer} from "seaport/test/TestTransferValidationZoneOfferer.sol";

import {StatefulTestZone} from "./impl/StatefulTestZone.sol";

import {AdditionalRecipient, AdvancedOrder, BasicOrderParameters, ConsiderationItem, CriteriaResolver, FulfillmentComponent, ItemType, OfferItem} from "seaport-types/src/lib/ConsiderationStructs.sol";

import {BasicOrderType, OrderType, Side} from "seaport-types/src/lib/ConsiderationEnums.sol";
...
contract PreAndPostFulfillmentCheckTest is BaseOrderTest {
  ...
  function execBasicStateful(Context memory context) public stateless {
    ...
    BasicOrderParameters memory basicOrderParameters = toBasicOrderParameters(
      baseOrderComponents,
    ...
    vm.warp(50);
    context.consideration.fulfillBasicOrder({parameters: basicOrderParameters});
  }
  ...
  function execFulfillAvailableAdvancedAscending(
    ...
    offerComponents.push(FulfillmentComponent({orderIndex: 0, itemIndex: 0}));
  ...
  function createMirrorOrder(
    ...
    configureOrderComponents(0);
    bytes32 orderHash = context.consideration.getOrderHash(baseOrderComponents);
    bytes memory signature = signOrder(context.consideration, pkey, orderHash);
