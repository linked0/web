...
import {ConduitTransfer, ConduitBatch1155Transfer, ConduitItemType} from "seaport-types/src/conduit/lib/ConduitStructs.sol";
...
contract ConduitExecuteWithBatch1155Test is BaseConduitTest {
  ...
  function testExecuteWithBatch1155(FuzzInputs memory inputs) public {
    ...
    for (uint8 i = 0; i < inputs.transferIntermediates.length; ++i) {
      transfers = extendConduitTransferArray(
        transfers,
        deployTokenAndCreateConduitTransfers(inputs.transferIntermediates[i])
      );
    }
  ...
  function executeWithBatch1155(Context memory context) external stateless {
    ...
    for (uint256 i = 0; i < context.batchTransfers.length; ++i) {
      ConduitBatch1155Transfer memory batchTransfer = context.batchTransfers[i];

      address[] memory toAddresses = new address[](batchTransfer.ids.length);
      for (uint256 j = 0; j < batchTransfer.ids.length; ++j) {
        toAddresses[j] = batchTransfer.to;
      }
      uint256[] memory actualBatchBalances = TestERC1155(batchTransfer.token)
        .balanceOfBatch(toAddresses, batchTransfer.ids);
      uint256[] memory expectedBatchBalances = getExpectedBatchTokenBalances(
        batchTransfer
      );
      assertTrue(actualBatchBalances.length == expectedBatchBalances.length);
