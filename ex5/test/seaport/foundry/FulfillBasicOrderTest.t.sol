...
import {AdditionalRecipient, Order, OrderComponents, BasicOrderParameters} from "seaport-types/src/lib/ConsiderationStructs.sol";

import {BaseOrderTest} from "./utils/BaseOrderTest.sol";

import {InvalidEthRecipient} from "../../contracts/test/InvalidEthRecipient.sol";
...
contract FulfillBasicOrderTest is BaseOrderTest, ConsiderationEventsAndErrors {
  ...
  function testBasicEthTo721WithAdditionalRecipients(
    ...
    AdditionalRecipient[] storage _additionalRecipients = additionalRecipients;
    for (uint256 i = 0; i < finalAdditionalRecipients; i++) {
      _additionalRecipients.push(
        AdditionalRecipient({recipient: alice, amount: inputs.paymentAmount})
      );
      addEthConsiderationItem(alice, inputs.paymentAmount);
    }
    _configureBasicOrderParametersEthTo721(inputs);
  ...
  function testFulfillBasicOrderRevertInvalidAdditionalRecipientsLength(
    ...
    for (
      ...
      _basicOrderParameters.additionalRecipients[i] = AdditionalRecipient({
        recipient: alice,
        amount: 1
      });
    }
  ...
  function revertDirtyUpperBitsForAdditionalRecipients(
    ...
    // Create basic order
    (, BasicOrderParameters memory _basicOrderParameters) = prepareBasicOrder(
      1
    );

    // Add additional recipients
    _basicOrderParameters.additionalRecipients = new AdditionalRecipient[](4);
  ...
  function revertUnusedItemParametersAddressSetOnNativeConsideration(
    ...
    BasicOrderParameters memory _basicOrderParameters = toBasicOrderParameters(
      baseOrderComponents,
    ...
    context.consideration.fulfillBasicOrder{value: 100}(_basicOrderParameters);
  }
  ...
  function revertInvalidTimestamp(Context memory context) external stateless {
    ...
    context.consideration.fulfillBasicOrder{value: context.args.paymentAmount}(
      basicOrderParameters
    );
  }
