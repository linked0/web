  ...
  ERC1155Recipient
{
  ...
  function _configureStructDefaults() internal {
    ...
    SeaportArrays
      .FulfillmentComponents(FulfillmentComponentLib.fromDefault(FIRST_FIRST))
      .saveDefaultMany(FIRST_FIRST);
    SeaportArrays
      .FulfillmentComponents(FulfillmentComponentLib.fromDefault(FIRST_SECOND))
      .saveDefaultMany(FIRST_SECOND);
    SeaportArrays
      .FulfillmentComponents(FulfillmentComponentLib.fromDefault(SECOND_FIRST))
      .saveDefaultMany(SECOND_FIRST);
    SeaportArrays
      .FulfillmentComponents(FulfillmentComponentLib.fromDefault(SECOND_SECOND))
    ...
    FulfillmentLib
      .empty()
      .withOfferComponents(FulfillmentComponentLib.fromDefaultMany(FIRST_FIRST))
      .withConsiderationComponents(
        FulfillmentComponentLib.fromDefaultMany(SECOND_FIRST)
      )
      .saveDefault(FF_SF);
  ...
  function createErc1155Token() internal returns (uint256 i) {
    ...
    erc1155s.push(token);
    setLabel(address(token), string.concat("ERC1155", LibString.toString(i)));
  }
