...
import {AdvancedOrder, ConsiderationItem, Execution, OrderParameters, ReceivedItem, SpentItem} from "seaport-sol/src/SeaportStructs.sol";
...
abstract contract FuzzSetup is Test, AmountDeriverHelper {
  ...
  function setUpContractOfferers(FuzzTestContext memory context) public pure {
    ...
    bytes32[2][] memory expectedContractOrderCalldataHashes = new bytes32[2][](
      context.executionState.orders.length
    );
    ...
    for (uint256 i = 0; i < context.executionState.orders.length; ++i) {
      ...
      if (
        ...
        registerChecks = true;
        expectedContractOrderCalldataHashes[i][0] = contractOrderCalldataHashes[
          i
        ][0];
        expectedContractOrderCalldataHashes[i][1] = contractOrderCalldataHashes[
          i
        ][1];
      }
    }
    ...
    for (uint256 i; i < context.executionState.orderDetails.length; ++i) {
      ...
      for (uint256 j = 0; j < items.length; j++) {
        ...
        if (item.itemType == ItemType.ERC20) {
          ...
          vm.prank(offerer);
          TestERC20(item.token).increaseAllowance(approveTo, item.amount);
        }
        ...
        if (item.itemType == ItemType.ERC1155) {
          TestERC1155(item.token).mint(offerer, item.identifier, item.amount);
          vm.prank(offerer);
    ...
    for (uint256 i; i < context.executionState.orderDetails.length; ++i) {
      ...
      for (uint256 j = 0; j < items.length; j++) {
        ...
        if (item.itemType == ItemType.ERC721) {
          ...
          if (
            context.executionState.caller == context.executionState.recipient ||
            ...
            for (
              ...
              if (
                context.executionState.orderDetails[k].unavailableReason !=
                UnavailableReason.AVAILABLE
              ) {
                continue;
              }
        ...
        if (item.itemType == ItemType.ERC1155) {
          TestERC1155(item.token).mint(owner, item.identifier, item.amount);
          vm.prank(owner);
          TestERC1155(item.token).setApprovalForAll(approveTo, true);
        }
      }
  ...
  function registerExpectedEventsAndBalances(
    ...
    Execution[] memory _executions = context.expectations.allExpectedExecutions;
    Execution[] memory executions = _executions;

    try balanceChecker.addTransfers(executions) {} catch (bytes memory reason) {
