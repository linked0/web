...
contract NativeBalances {
  ...
  function addNativeTransfer(address from, address to, uint256 amount) public {
...
contract ERC721Balances {
  ...
  function addERC721Transfer(
    ...
    (bool fromExists, uint256 fromBalance) = tokenData.accountBalances.tryGet(
      from
    );
    if (!fromExists) {
  ...
  function checkERC1155Balances() internal view {
    ...
    for (uint256 i; i < length; i++) {
      ...
      for (uint256 j; j < accountsLength; j++) {
        ...
        EnumerableMap.UintToUintMap storage accountIdentifiers = tokenData
          .accountIdentifiers[account];
  ...
  ERC1155Balances
{
  function addTransfer(Execution calldata execution) public {
    ...
    if (item.itemType == ItemType.NATIVE) {
      return addNativeTransfer(execution.offerer, item.recipient, item.amount);
    }
