...
import {AdvancedOrder, ConsiderationItem, OfferItem, OrderParameters, ReceivedItem, SpentItem} from "seaport-sol/src/SeaportStructs.sol";
...
import {OrderStatusEnum, ContractOrderRebate} from "seaport-sol/src/SpaceEnums.sol";
...
import {FractionResults, FractionStatus, FractionUtil} from "./FractionUtil.sol";
...
abstract contract FuzzAmendments is Test {
    ...
    for (uint256 i = 0; i < context.executionState.orders.length; ++i) {
      ...
      if (orderParams.orderType == OrderType.CONTRACT) {
        ...
        HashCalldataContractOfferer offerer;
        {
          ...
          offerer = (HashCalldataContractOfferer(payable(orderParams.offerer)));
          ...
          if (rebate == ContractOrderRebate.MORE_OFFER_ITEMS) {
          ...
          } else if (rebate == ContractOrderRebate.MORE_OFFER_ITEM_AMOUNTS) {
            uint256 itemIdx = _findFirstNon721Index(orderParams.offer);
          ...
          } else if (rebate == ContractOrderRebate.LESS_CONSIDERATION_ITEMS) {
            ...
            uint256 itemIdx = _findFirstNon721Index(orderParams.consideration);
            offerer.addItemAmountMutation(
              Side.CONSIDERATION,
              itemIdx,
          ...
          uint256 shiftedOfferer = (uint256(uint160(orderParams.offerer)) <<
            96);
        ...
        context.executionState.previewedOrders[i].parameters.offer = _toOffer(
          offer
        );
        context
          .executionState
  ...
  function setPartialFills(FuzzTestContext memory context) public {
    for (uint256 i = 0; i < context.executionState.orders.length; ++i) {
      ...
      if (
        ...
        revert("FuzzAmendments: invalid order type for partial fill state");
      }
    ...
    for (
      ...
      if (
        ...
        context
          .executionState
          .orders[i]
          .inscribeOrderStatusNumeratorAndDenominator(1, 1, context.seaport);
      } else if (
        context.executionState.preExecOrderStatuses[i] ==
        OrderStatusEnum.AVAILABLE
      ) {
        context
          .executionState
          .orders[i]
          .inscribeOrderStatusNumeratorAndDenominator(0, 0, context.seaport);
        context.executionState.orders[i].inscribeOrderStatusCancelled(
          false,
          context.seaport
        );
      } else if (
        context.executionState.preExecOrderStatuses[i] == OrderStatusEnum.REVERT
      ) {
  ...
  function setCounter(FuzzTestContext memory context) public {
    for (uint256 i = 0; i < context.executionState.orders.length; ++i) {
      ...
      order.offerer.inscribeCounter(offererSpecificCounter, context.seaport);
    }
  }
  ...
  function _findFirstNon721Index(
    ...
    revert("FuzzAmendments: could not locate non-721 consideration item index");
  }
