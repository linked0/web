...
import {AdvancedOrder, ConsiderationItem, CriteriaResolver, Execution, OfferItem} from "seaport-sol/src/SeaportStructs.sol";
...
library MutationEligibilityLib {
  ...
  function withGeneric(
    Failure failure,
    function(FuzzTestContext memory) internal returns (bool) ineligibilityFilter
  ) internal pure returns (IneligibilityFilter memory) {
    return
      IneligibilityFilter(
        failure.one(),
  ...
  function setAllIneligibleFailures(
    ...
    for (uint256 i = 0; i < failuresAndFilters.length; ++i) {
      IneligibilityFilter memory failuresAndFilter = (failuresAndFilters[i]);

      if (
        failuresAndFilter.derivationMethod == MutationContextDerivation.GENERIC
      ) {
        ...
        failuresAndFilter.derivationMethod == MutationContextDerivation.ORDER
      ) {
        setIneligibleFailures(
          context,
  ...
  function hasNoEligibleOrders(
    ...
    for (uint256 i; i < context.executionState.orders.length; i++) {
      ...
      if (!ineligibleCondition(context.executionState.orders[i], i, context)) {
        return false;
      }
    }
  ...
  function hasNoEligibleFailures(
    FuzzTestContext memory context,
    function(FuzzTestContext memory) internal returns (bool) ineligibleCondition
  ) internal returns (bool) {
  ...
  function getEligibleIndex(
    ...
    uint256[] memory eligibleIndexes = new uint256[](eligibleElements.length);
  ...
  function deriveMutationContext(
    ...
    if (derivationMethod == MutationContextDerivation.ORDER) {
      ...
      mutationState.selectedArbitraryAddress = address(
        uint160(context.generatorContext.randRange(3, type(uint160).max))
      );
    } else if (
...
library Failarray {
  ...
  function and(Failure a, Failure b) internal pure returns (Failure[] memory) {
