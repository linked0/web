...
import {AdvancedOrder, ConsiderationItem, CriteriaResolver, Execution, Fulfillment, FulfillmentComponent, OfferItem, OrderParameters, ReceivedItem, SpentItem} from "seaport-sol/src/SeaportStructs.sol";

import {ItemType, OrderType, Side} from "seaport-sol/src/SeaportEnums.sol";

import {AdvancedOrderLib, OrderParametersLib, ConsiderationItemLib, ItemType, BasicOrderType, ConsiderationItemLib} from "seaport-sol/src/SeaportSol.sol";

import {FulfillmentDetails, OrderDetails} from "seaport-sol/src/fulfillments/lib/Structs.sol";

import {ContractOrderRebate, UnavailableReason} from "seaport-sol/src/SpaceEnums.sol";

import {FractionStatus, FractionUtil} from "./FractionUtil.sol";

import {AdvancedOrdersSpaceGenerator, Offerer, SignatureMethod} from "./FuzzGenerators.sol";
...
import {MutationEligibilityLib, MutationHelpersLib} from "./FuzzMutationHelpers.sol";

import {EIP1271Offerer} from "./EIP1271Offerer.sol";

import {HashCalldataContractOfferer} from "../../../../contracts/test/HashCalldataContractOfferer.sol";

import {HashValidationZoneOfferer} from "../../../../contracts/test/HashValidationZoneOfferer.sol";

import {OffererZoneFailureReason} from "../../../../contracts/test/OffererZoneFailureReason.sol";
...
library MutationFilters {
  ...
  function ineligibleWhenNotActiveTimeOrNotContractOrder(
    ...
    ).failureReasons(context.executionState.orderDetails[orderIndex].orderHash);
  ...
  function ineligibleWhenNotActiveTimeOrNotContractOrderOrNoOffer(
    ...
    return
      ineligibleWhenNotActiveTimeOrNotContractOrder(order, orderIndex, context);
  }
  ...
  function ineligibleWhenOrderHasRebates(
    ...
    if (order.parameters.orderType == OrderType.CONTRACT) {
      if (
        ...
        context.executionState.orderDetails[orderIndex].consideration.length !=
  ...
  function ineligibleForBadFraction(
    ...
    return
      ineligibleWhenNotAvailableOrContractOrder(order, orderIndex, context);
  }
  ...
  function ineligibleForOrderAlreadyFilled(
    ...
    if (
      ineligibleWhenFulfillAvailable(context) || ineligibleWhenBasic(context)
    ) {
      return true;
    }
  ...
  function ineligibleForMissingItemAmount_OfferItem_FulfillAvailable(
    ...
    for (uint256 i; i < context.executionState.offerFulfillments.length; i++) {
  ...
  function ineligibleForMissingItemAmount_OfferItem(
    ...
    if (
      ineligibleWhenFulfillAvailable(context) || ineligibleWhenMatch(context)
    ) {
      return true;
    }
  ...
  function ineligibleForUnusedItemParameters_Identifier(
    ...
    for (uint256 i; i < order.parameters.consideration.length; i++) {
      ConsiderationItem memory item = order.parameters.consideration[i];
      if (item.itemType == ItemType.ERC20 || item.itemType == ItemType.NATIVE) {
        return false;
      }
    }
  ...
  function ineligibleForInexactFraction(
    ...
    if (
      order.parameters.offer.length + order.parameters.consideration.length == 0
    ) {
      return true;
    }
...
contract FuzzMutations is Test, FuzzExecutor {
  ...
  function mutation_mismatchedFulfillmentOfferAndConsiderationComponents_Swapped(
    ...
    FulfillmentComponent memory firstOfferComponent = (firstOfferComponents[0]);
  ...
  function mutation_invalidWildcardProof(
    ...
    CriteriaResolver memory resolver = context.executionState.criteriaResolvers[
      ...
      if (
        ...
        if (
          ...
          order
            ...
            .offer[fulfillmentComponent.itemIndex]
            .startAmount = 0;
          order.parameters.offer[fulfillmentComponent.itemIndex].endAmount = 0;
          ...
          _signOrValidateMutatedOrder(context, fulfillmentComponent.orderIndex);
  ...
  function mutation_missingItemAmount_ConsiderationItem(
    ...
    order
      ...
      .startAmount = 0;
    order.parameters.consideration[firstNon721ConsiderationItem].endAmount = 0;
  ...
  function mutation_noContract(
    ...
    for (uint256 i; i < context.executionState.orders.length; i++) {
      ...
      for (uint256 j; j < order.parameters.consideration.length; j++) {
        ConsiderationItem memory item = order.parameters.consideration[j];
  ...
  function mutation_unusedItemParameters_Identifier(
    ...
    if (!validItemFound) {
      for (uint256 i; i < order.parameters.consideration.length; i++) {
        ConsiderationItem memory item = order.parameters.consideration[i];
        if (
          item.itemType == ItemType.ERC20 || item.itemType == ItemType.NATIVE
        ) {
          item.identifierOrCriteria = 1;
          validItemFound = true;
          break;
