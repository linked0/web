...
import {AdvancedOrderLib, ConsiderationItemLib, MatchComponent, MatchComponentType, OfferItemLib, OrderComponentsLib, OrderLib, OrderParametersLib, SeaportArrays, ZoneParametersLib} from "seaport-sol/src/SeaportSol.sol";

import {AdvancedOrder, ConsiderationItem, CriteriaResolver, Fulfillment, OfferItem, Order, OrderComponents, OrderParameters, ReceivedItem, SpentItem, ZoneParameters} from "seaport-sol/src/SeaportStructs.sol";

import {BasicOrderRouteType, BasicOrderType, ItemType, OrderType, Side} from "seaport-sol/src/SeaportEnums.sol";

import {UnavailableReason} from "seaport-sol/src/SpaceEnums.sol";

import {ContractOffererInterface} from "seaport-sol/src/ContractOffererInterface.sol";
...
library FuzzHelpers {
  ...
  function findSmallestDenominator(
    ...
    require(numbers.length > 0, "FuzzHelpers: Input array must not be empty");
  ...
  function getSmallestDenominator(
    ...
    if (totalFractionalizableAmounts != 0) {
      uint256[] memory numbers = new uint256[](totalFractionalizableAmounts);
  ...
  function getBasicOrderTypeEligibility(
    ...
    OrderComponents memory orderComponents = order.parameters.toOrderComponents(
      counter
    );
  ...
  function getExpectedContractOffererCalldataHashes(
    ...
    for (uint256 i; i < orders.length; ++i) {
      ...
      uint256 shiftedOfferer = uint256(uint160(order.parameters.offerer)) << 96;
  ...
  function _checkCriteria(
    ...
    for (uint256 i; i < consideration.length; ++i) {
      ...
      hasCriteria = (itemType == ItemType.ERC721_WITH_CRITERIA ||
        itemType == ItemType.ERC1155_WITH_CRITERIA);
      if (hasCriteria) {
        return (hasCriteria, considerationItem.identifierOrCriteria != 0);
      }
    }
