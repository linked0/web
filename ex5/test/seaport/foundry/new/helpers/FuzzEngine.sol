...
import {AdvancedOrderLib, FulfillAvailableHelper, MatchFulfillmentHelper, OrderComponentsLib, OrderLib, OrderParametersLib, OfferItem, ConsiderationItem, ItemType, OrderType} from "seaport-sol/src/SeaportSol.sol";

import {AdvancedOrder, BasicOrderParameters, Execution, Order, OrderComponents, OrderParameters} from "seaport-sol/src/SeaportStructs.sol";
...
import {FuzzTestContext, FuzzTestContextLib, FuzzParams, MutationState} from "./FuzzTestContextLib.sol";

import {AdvancedOrdersSpace, AdvancedOrdersSpaceGenerator, TestStateGenerator} from "./FuzzGenerators.sol";
...
import {ErrorsAndWarnings, ValidationConfiguration} from "../../../../contracts/helpers/order-validator/SeaportValidator.sol";

import {IssueStringHelpers} from "../../../../contracts/helpers/order-validator/lib/SeaportValidatorTypes.sol";

import {NavigatorRequest} from "../../../../contracts/helpers/navigator/lib/SeaportNavigatorTypes.sol";

import {NavigatorAdvancedOrderLib} from "../../../../contracts/helpers/navigator/lib/NavigatorAdvancedOrderLib.sol";

import {FulfillmentStrategy, AggregationStrategy, FulfillAvailableStrategy, MatchStrategy} from "seaport-sol/src/fulfillments/lib/FulfillmentLib.sol";
  ...
  MatchFulfillmentHelper
{
  ...
  function generate(
    ...
    if (orders.getStructure(address(context.seaport)) == Structure.ADVANCED) {
      context = context.withRecipient(
        AdvancedOrdersSpaceGenerator.generateRecipient(space, generatorContext)
      );
    }
  ...
  function execFailure(FuzzTestContext memory context) internal {
    ...
    assertFalse(success, string.concat("Mutation ", name, " did not revert"));
    ...
    if (vm.envOr("SEAPORT_FUZZ_NAVIGATOR", false)) {
      ...
      if (!isContractOrder) {
        FulfillmentStrategy memory fulfillmentStrategy = FulfillmentStrategy({
          ...
          fulfillAvailableStrategy: FulfillAvailableStrategy.DROP_RANDOM_OFFER,
