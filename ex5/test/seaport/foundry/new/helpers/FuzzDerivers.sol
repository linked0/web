...
import {AdvancedOrderLib, MatchComponent, MatchComponentType} from "seaport-sol/src/SeaportSol.sol";

import {AdvancedOrder, CriteriaResolver, Execution, Fulfillment, FulfillmentComponent, OfferItem, OrderParameters, ReceivedItem, SpentItem} from "seaport-sol/src/SeaportStructs.sol";

import {ItemType, OrderType} from "seaport-sol/src/SeaportEnums.sol";

import {ItemType} from "seaport-sol/src/SeaportEnums.sol";

import {OrderStatusEnum, UnavailableReason} from "seaport-sol/src/SpaceEnums.sol";
...
import {FulfillmentGeneratorLib} from "seaport-sol/src/fulfillments/lib/FulfillmentLib.sol";
...
library FuzzDerivers {
  ...
  function withDerivedOrderDetails(
    ...
    bytes32[] memory orderHashes = context.executionState.orders.getOrderHashes(
    ...
    for (uint256 i; i < context.executionState.orders.length; ++i) {
      OrderParameters memory order = context
        ...
        .orders[i]
        .parameters;
      OrderStatusEnum status = context.executionState.preExecOrderStatuses[i];
      ...
      if (
        ...
        if (!(block.timestamp < order.endTime)) {
          ...
          context
            .executionState
            .orderDetails[i]
            .unavailableReason = UnavailableReason.ALREADY_FULFILLED;
        } else if (status == OrderStatusEnum.REVERT) {
          context
            .executionState
            .orderDetails[i]
            .unavailableReason = UnavailableReason.GENERATE_ORDER_FAILURE;
        } else if (
          ...
          context
            .executionState
            .orderDetails[i]
            .unavailableReason = UnavailableReason.MAX_FULFILLED_SATISFIED;
        } else {
          totalAvailable += 1;
        }
      }
  ...
  function withDerivedFulfillments(
    ...
    context.executionState.remainingOfferComponents = remainingOfferComponents
    ...
    )
  {
    ...
    if (
      ...
      if (explicitExecutions.length == 0) {
        revert("FuzzDerivers: no explicit execs derived - fulfillAvailable");
      }
    } else if (
    ...
    )
  {
    if (
      ...
      if (explicitExecutions.length == 0) {
        revert("FuzzDerivers: no explicit execs (direct) - fulfillAvailable");
      }
    } else if (
  ...
  function withDerivedExecutions(
    ...
    context.expectations.expectedImplicitPreExecutions = implicitExecutionsPre;
    context
      .expectations
