...
import {AdvancedOrder, Execution, OrderParameters, OrderType} from "seaport-sol/src/SeaportStructs.sol";
...
abstract contract FuzzChecks is Test {
  ...
  function check_contractOrderExpectedDataHashes(
    ...
    for (uint256 i; i < context.executionState.orders.length; i++) {
      ...
      if (order.parameters.orderType == OrderType.CONTRACT) {
        ...
        bytes32 expectedGenerateOrderCalldataHash = expectedCalldataHashes[i][
          0
        ];

        bytes32 expectedRatifyOrderCalldataHash = expectedCalldataHashes[i][1];
  ...
  function check_executions(FuzzTestContext memory context) public {
    ...
    for (
      ...
      assertEq(actual.offerer, expected.offerer, "check_executions: offerer");
    }
  }
   ...
   * @param context A Fuzz test context.
   */
  function check_expectedBalances(FuzzTestContext memory context) public view {
  ...
  function check_orderStatusFullyFilled(FuzzTestContext memory context) public {
    for (uint256 i; i < context.executionState.orders.length; i++) {
      ...
      if (
        ...
        assertEq(totalSize, 1, "check_orderStatusFullyFilled: totalSize != 1");
      } else if (
        context.executionState.preExecOrderStatuses[i] ==
        ...
        if (order.parameters.orderType == OrderType.CONTRACT) {
          ...
          uint256 currentNonce = context.seaport.getContractOffererNonce(
