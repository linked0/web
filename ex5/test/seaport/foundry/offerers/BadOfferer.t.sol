...
import {OfferItem, ConsiderationItem, AdvancedOrder, CriteriaResolver, OrderParameters, FulfillmentComponent} from "seaport-types/src/lib/ConsiderationStructs.sol";
...
contract BadOffererTest is BaseOrderTest, ZoneInteractionErrors {
  ...
  function testNormalOrder() public {
    ...
    test(
      ...
      Context({seaport: consideration, id: id, eoa: false, shouldFail: false})
    );
    test(
      this.execOrderWithContext,
      Context({
  ...
  function testOrderNothing() public {
    ...
    test(
      this.execOrderWithContext,
      Context({seaport: consideration, id: id, eoa: false, shouldFail: true})
    );
    test(
      this.execOrderWithContext,
      Context({
  ...
  function testOrderEoa() public {
    ...
    test(
      this.execOrderWithContext,
      Context({seaport: consideration, id: id, eoa: true, shouldFail: true})
    );
    test(
      this.execOrderWithContext,
      Context({
