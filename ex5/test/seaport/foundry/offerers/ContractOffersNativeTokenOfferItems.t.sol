...
import {AdvancedOrder, OrderParameters, OrderComponents, CriteriaResolver, SpentItem, ReceivedItem} from "seaport-types/src/lib/ConsiderationStructs.sol";

import {ContractOffererInterface} from "seaport-types/src/interfaces/ContractOffererInterface.sol";

import {TestContractOffererNativeToken} from "../../../contracts/test/TestContractOffererNativeToken.sol";
  ...
  BaseOrderTest
{
  ...
  function testEthForErc721(FuzzArgs memory args) public validateInputs(args) {
    test(
      this.ethForErc721,
      Context({
  ...
  function ethForErc721(Context memory context) public stateless {
    ...
    assertEq(context.args.ethAmount, address(this).balance - originalBalance);
    assertEq(address(contractOfferer), test721_1.ownerOf(context.args.nftId));
  }
}
