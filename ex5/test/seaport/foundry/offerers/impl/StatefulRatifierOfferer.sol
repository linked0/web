...
import {SpentItem, ReceivedItem, Schema} from "seaport-types/src/lib/ConsiderationStructs.sol";
  ...
  function ratifyOrder(
    ...
    for (uint256 i = 0; i < minimumReceived.length; i++) {
      if (minimumReceived[i].itemType != ItemType.ERC20) {
        revert IncorrectItemType(minimumReceived[i].itemType, ItemType.ERC20);
      }
    ...
    for (uint256 i; i < maximumSpent.length; i++) {
      if (maximumSpent[i].itemType != ItemType.ERC721) {
        revert IncorrectItemType(maximumSpent[i].itemType, ItemType.ERC721);
      }
