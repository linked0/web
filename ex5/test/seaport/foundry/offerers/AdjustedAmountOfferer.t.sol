...
import {ERC20Interface} from "seaport-types/src/interfaces/AbridgedTokenInterfaces.sol";

import {ConsiderationInterface} from "seaport-types/src/interfaces/ConsiderationInterface.sol";

import {OrderType, ItemType} from "seaport-types/src/lib/ConsiderationEnums.sol";

import {ConsiderationItem, AdvancedOrder, CriteriaResolver} from "seaport-types/src/lib/ConsiderationStructs.sol";
  ...
  ZoneInteractionErrors
{
  ...
  function execLessMinimumReceived(Context memory context) external stateless {
    vm.expectRevert(
  ...
  function execMoreMinimumReceived(Context memory context) external stateless {
  ...
  function testLessMaximumSpent() public {
    ...
    test(this.execLessMaximumSpent, Context({seaport: referenceConsideration}));
  }
  ...
  function testMoreMaximumSpent() public {
    ...
    test(this.execMoreMaximumSpent, Context({seaport: referenceConsideration}));
  }
