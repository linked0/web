...
import {ConsiderationItem, OfferItem, Order, OrderComponents, OrderParameters, OrderType} from "seaport-types/src/lib/ConsiderationStructs.sol";
...
contract BulkSignatureTest is BaseOrderTest {
  ...
  function execBulkSignatureSparse(Context memory context) external stateless {
    string memory offerer = "offerer";
    (address addr, uint256 key) = makeAddrAndKey(offerer);
    addErc721OfferItem(1);
    test721_1.mint(address(addr), 1);
  ...
  function testBulkSignatureSparseFuzz(SparseArgs memory sparseArgs) public {
    ...
    test(
      this.execBulkSignatureSparse,
      Context({seaport: consideration, args: sparseArgs, useCompact2098: false})
    );
    test(
      this.execBulkSignatureSparse,
      Context({
    ...
    test(
      this.execBulkSignatureSparse,
      Context({seaport: consideration, args: sparseArgs, useCompact2098: true})
    );
    test(
      this.execBulkSignatureSparse,
      Context({
    ...
    )
  {
    ...
    secondOfferItem = OfferItem(ItemType.ERC721, address(test721_1), 2, 1, 1);
