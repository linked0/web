...
import {ConduitMockInvalidMagic} from "../../contracts/test/ConduitMockInvalidMagic.sol";

import {ConduitMockRevertNoReason} from "../../contracts/test/ConduitMockRevertNoReason.sol";
...
contract TransferHelperSingleRecipientTest is BaseOrderTest {
  ...
  function setUp() public override {
    ...
    for (
      ...
      for (uint256 tokenIdx = 0; tokenIdx < erc1155s.length; tokenIdx++) {
        erc1155s[tokenIdx].mint(alice, identifier, TOTAL_FUNGIBLE_TOKENS);
      }
    }
  ...
  function _balanceOfTransferItemForAddress(
    ...
    if (item.itemType == ConduitItemType.ERC20) {
      ...
      return TestERC721(item.token).ownerOf(item.identifier) == addr ? 1 : 0;
  ...
  function _getTransferHelperItemsWithRecipientFromTransferHelperItems(
    ...
    TransferHelperItemsWithRecipient[]
      memory itemsWithRecipient = new TransferHelperItemsWithRecipient[](1);
    itemsWithRecipient[0] = TransferHelperItemsWithRecipient(items, to, true);
  ...
  function _performMultiItemTransferAndCheckBalances(
    ...
    if (expectRevertData.length > 0) {
      ...
      for (uint256 i = 0; i < items.length; i++) {
        assert(beforeTransferBalances[i].from == afterTransferBalances[i].from);
        assert(beforeTransferBalances[i].to == afterTransferBalances[i].to);
      }
      return;
    }
  ...
  function _getFuzzedTransferItem(
    ...
    if (itemType == ConduitItemType.ERC20) {
      ...
      return TransferHelperItem(itemType, address(erc1155), identifier, amount);
      ...
      return
        TransferHelperItem(itemType, address(erc721s[index]), identifier, 1);
    }
    revert();
  }
  ...
  function testRevertBulkTransferERC20InvalidIdentifier(
    ...
    _performSingleItemTransferAndCheckBalances(
      ...
      alice,
      bob,
      abi.encodePacked(TransferHelperErrors.InvalidERC20Identifier.selector)
    );
  }
  ...
  function testRevertBulkTransferUnknownConduit(
    ...
    vm.assume(fuzzConduitKey != bytes32(0) && fuzzConduitKey != conduitKeyOne);
  ...
  function testRevertInvalidConduitMagicValue() public {
    ...
    bytes32 conduitKeyAlice = bytes32(uint256(uint160(address(alice))) << 96);
    ...
    if (!coverage_or_debug) {
      ...
      mockTransferHelper = new TransferHelper(address(mockConduitController));
    }
    ...
    bytes32 mockConduitKey = mockConduitController.getKey(address(mockConduit));
    ...
    (address derivedConduit, bool exists) = mockConduitController.getConduit(
      conduitKeyAlice
    );
