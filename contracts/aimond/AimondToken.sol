...
contract Lockable is Context {
    ...
    function getLockedAmount(address _to) public view returns (uint256) {
        return _lockedAmount[_to];
    }

    function _increaseLockAmount(address to, uint256 balance) internal virtual {
        require(to != address(0), "ERC20: lock to the zero address");

        _lockedAmount[to] += balance;
        emit Locked(to, balance);
    }

    function _decreaseLockAmount(address to, uint256 balance) internal virtual {
        require(to != address(0), "ERC20: lock to the zero address");
        require(
            _lockedAmount[to] >= balance,
            "ERC20: cannot unlock more than locked"
        );
        _lockedAmount[to] -= balance;
        emit Unlocked(to, balance);
    }
}
...
contract ERC20 is Context, IERC20, IERC20Metadata, Lockable {
    ...
    function _transfer(
        ...
        uint256 freeBalance = fromBalance - getLockedAmount(from);
        require(
            freeBalance >= amount,
            "ERC20: transfer amount exceeds free balance"
        );
        unchecked {
...
contract ERC20Base is Context, ERC20, Ownable {
    ...
    function lock(address to, uint256 amount) public onlyOwner {
        _increaseLockAmount(to, amount);
    }

    function unlock(address to, uint256 amount) public onlyOwner {
        _decreaseLockAmount(to, amount);
    }
    ...
    function transferLockUp(
        ...
        bool status = _Transfer(to, amount);
        lock(to, amount);
        return status;
    }
    ...
    function transferFromLockUp(
        ...
        bool status = _TransferFrom(from, to, amount);
        lock(to, amount);
        return status;
    }
...
contract Aimond is ERC20Base, ReentrancyGuard {
    ...
    function mint(uint256 amount) public onlyOwner {
        require(
            getTotalSupply() + amount <= maxSupply,
            "Minting specified amount would exceed maximum cap"
        );
