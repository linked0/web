...
import {AdvancedOrder, CriteriaResolver, OfferItem, ConsiderationItem, OrderParameters} from "../../../seaport-types/src/lib/ConsiderationStructs.sol";
...
import {NavigatorAdvancedOrder, NavigatorConsiderationItem, NavigatorOfferItem, NavigatorOrderParameters} from "./SeaportNavigatorTypes.sol";
...
library NavigatorAdvancedOrderLib {
  ...
  function fromAdvancedOrders(
    ...
    NavigatorAdvancedOrder[] memory helperOrders = new NavigatorAdvancedOrder[](
      orders.length
    );
  ...
  function toAdvancedOrder(
    ...
    CriteriaResolver[] memory criteriaResolvers = new CriteriaResolver[](
      order.parameters.offer.length + order.parameters.consideration.length
    );
    ...
    // array.
    OfferItem[] memory offer = new OfferItem[](order.parameters.offer.length);
    for (uint256 i; i < order.parameters.offer.length; i++) {
    ...
    for (uint256 i; i < order.parameters.consideration.length; i++) {
      NavigatorConsiderationItem memory item = order.parameters.consideration[
        i
      ];
      if (item.hasCriteria()) {
        item.validate();
