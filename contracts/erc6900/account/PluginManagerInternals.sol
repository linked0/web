...
abstract contract PluginManagerInternals is IPluginManager {
  ...
  function _installPlugin(
    ...
    for (uint256 i = 0; i < length; ) {
      ...
      _storage.callPermitted[
        getPermittedCallKey(plugin, manifest.permittedExecutionSelectors[i])
      ] = true;
    ...
    if (manifest.permitAnyExternalAddress) {
      ...
      for (uint256 i = 0; i < length; ) {
        ManifestExternalCallPermission memory externalCallPermission = manifest
          .permittedExternalCalls[i];

        PermittedExternalCallData storage permittedExternalCallData = _storage
    ...
    for (uint256 i = 0; i < length; ) {
      ManifestAssociatedFunction memory mv = manifest.userOpValidationFunctions[
        i
      ];
    ...
    for (uint256 i = 0; i < length; ) {
      ManifestAssociatedFunction memory mh = manifest.preUserOpValidationHooks[
        i
      ];
    ...
    for (uint256 i = 0; i < length; ) {
      ManifestAssociatedFunction memory mh = manifest.preRuntimeValidationHooks[
        i
      ];
  ...
  function _resolveManifestFunction(
    ...
    if (manifestFunction.functionType == ManifestAssociatedFunctionType.SELF) {
      ...
      if (
        allowedMagicValue == ManifestAssociatedFunctionType.PRE_HOOK_ALWAYS_DENY
      ) {
