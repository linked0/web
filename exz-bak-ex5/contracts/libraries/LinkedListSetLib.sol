...
library LinkedListSetLib {
  ...
  function tryRemoveKnown(
    ...
    _store(
      prevSlot,
      clearFlags(next) | getUserFlags(currentValue) | (next & HAS_NEXT_FLAG)
    );
    _store(valueSlot, bytes32(0));
  ...
  function getAll(
    AssociatedLinkedListSet storage set,
    address associated
  ) public view returns (SetValue[] memory res) {
    TempBytesMemory keyBuffer = _allocateTempKeyBuffer(set, associated);
  ...
  function isSentinel(bytes32 value) public pure returns (bool ret) {
    assembly ("memory-safe") {
      ret := and(value, 1)
    }
  }

  function hasNext(bytes32 value) public pure returns (bool) {
    return value & HAS_NEXT_FLAG != 0;
  }

  function clearFlags(bytes32 val) public pure returns (bytes32) {
    return
      val & 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0001;
  }

  /// @dev Preserves the lower two bits
  function clearUserFlags(bytes32 val) public pure returns (bytes32) {
    return
      val & 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0003;
  }

  function getUserFlags(bytes32 val) public pure returns (bytes32) {
