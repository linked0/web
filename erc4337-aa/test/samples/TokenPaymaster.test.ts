...
describe("TokenPaymaster", function () {
  ...
  // NOTE: This test is chagne to succeed with Jay's changes
  // In order to succeed, the deployer sends some tokens to the account
  it.only("REVERSE: paymaster should reject if account does not have enough tokens or allowance", async () => {
    const snapshot = await ethers.provider.send("evm_snapshot", []);
    let op = await fillUserOp(
      {
        sender: account.address,
    ...
    const ret = await entryPoint.handleOps([opPacked], beneficiaryAddress, {
      gasLimit: 1e7,
    });
    console.log(ret);
