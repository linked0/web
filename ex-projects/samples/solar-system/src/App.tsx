...
function CameraTracker({ activePlanet, planets }: CameraTrackerProps) {
    const { camera, controls } = useThree() as any
    const vec = new THREE.Vector3()

    useFrame((state) => {
        if (activePlanet) {
            const planetData = planets.find(p => p.name === activePlanet)
            if (planetData) {
                // Calculate current position of the planet based on time and speed
                const t = state.clock.getElapsedTime() * planetData.speed
                const x = Math.cos(t) * planetData.distance
                const z = -Math.sin(t) * planetData.distance

                vec.set(x, 0, z)

                // Smoothly transition the target
                controls.target.lerp(vec, 0.1)

                // Adjust camera distance for better view of the specific planet
                const idealDistance = planetData.size * 5 + 5
                const currentDistance = camera.position.distanceTo(controls.target)
                if (currentDistance > idealDistance + 10) {
                    camera.position.lerp(
                        controls.target.clone().add(
                            camera.position.clone().sub(controls.target).normalize().multiplyScalar(idealDistance + 5)
                        ),
                        0.05
                    )
                }
            ...
            // Return to sun (center)
            vec.set(0, 0, 0)
            controls.target.lerp(vec, 0.05)
        }
    })
        ...
        <div className="w-full h-screen bg-black overflow-hidden relative font-sans">
                    ...
                    >
                        <div className="glass p-6 rounded-2xl w-64 shadow-2xl">
                            <div className="flex items-start justify-between mb-4">
                                <div>
                                    <h2 className="text-2xl font-bold text-white">{activePlanet}</h2>
            ...
            <Canvas shadows gl={{ antialias: false }}>
                <Suspense fallback={null}>
                    <CameraTracker activePlanet={activePlanet} planets={PLANETS} />
                    ...
                    {PLANETS.map((planet) => (
                        ...
                        >
                            {planet.name === 'Earth' && (
                                <group position={[0, planet.size + 0.1, 0]}>
                                    <Human />
                                </group>
                            )}
                        </Planet>
