...
contract TransactionDelegator {
    ...
    mapping(address => mapping(uint256 => MintRequest)) public requests;
    ...
    function mint(address _user, uint256 _id) public {
        ...
        require(requests[_user][_id].isSet == false, "TransactionDelegator: request already exists");
        // save the request to the mapping
        requests[_user][_id] = MintRequest({id: _id, userAddress: _user, status: MintStatus.PENDING, isSet: true});
    ...
    function getStatus(address _user, uint256 _id) public view returns (MintStatus) {
