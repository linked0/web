...
import {PluginStorageLib, StoragePointer} from "@erc6900/libraries/PluginStorageLib.sol";
...
contract PluginStorageLibTest is Test {
    ...
    function test_storagePointer() public {
        bytes memory key = PluginStorageLib.allocateAssociatedStorageKey(
            account1,
            0,
            1
        );
        ...
        assertEq(
            vm.load(address(this), expectedKey),
            bytes32(uint256(0xdeadbeef))
        );
        assertEq(accountWrites[1], bytes32(uint256(expectedKey) + 1));
        assertEq(
            vm.load(address(this), bytes32(uint256(expectedKey) + 1)),
            bytes32(uint256(123))
        );
    }
    ...
    function testFuzz_storagePointer(
        ...
        uint256[FUZZ_ARR_SIZE] storage val = _castPtrToArray(
            PluginStorageLib.associatedStorageLookup(key, inputKey)
        );
    ...
    function _castPtrToArray(
        StoragePointer ptr
    ) internal pure returns (uint256[FUZZ_ARR_SIZE] storage val) {
    ...
    function _castPtrToStruct(
        StoragePointer ptr
    ) internal pure returns (TestStruct storage val) {
        assembly ("memory-safe") {
            val.slot := ptr
        }
    }
