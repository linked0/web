...
import {_ACCOUNT_STORAGE_SLOT, getPermittedCallKey} from "@erc6900/account/AccountStorage.sol";
...
contract AccountStorageTest is Test {
    ...
    function test_storageSlotImpl() external {
        ...
        assertEq(
            uint256(vm.load(address(impl), _ACCOUNT_STORAGE_SLOT)),
            type(uint8).max
        );

        // should revert if we try to initialize again
        vm.expectRevert(
            AccountStorageInitializable.AlreadyInitialized.selector
        );
        impl.initialize();
    }
