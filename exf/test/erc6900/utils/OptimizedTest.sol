...
abstract contract OptimizedTest is Test {
    function _isOptimizedTest() internal returns (bool) {
        ...
        return
            _isStringEq(profile, "optimized-test-deep") ||
            _isStringEq(profile, "optimized-test");
    }

    function _isStringEq(
        string memory a,
        string memory b
    ...
    function _deploySingleOwnerPlugin() internal returns (SingleOwnerPlugin) {
        return
            _isOptimizedTest()
                ? SingleOwnerPlugin(
                    deployCode(
                        "out-optimized/SingleOwnerPlugin.sol/SingleOwnerPlugin.json"
                    )
                )
                : new SingleOwnerPlugin();
    }

    function _deployTokenReceiverPlugin()
        internal
        returns (TokenReceiverPlugin)
    {
        return
            _isOptimizedTest()
                ? TokenReceiverPlugin(
                    deployCode(
                        "out-optimized/TokenReceiverPlugin.sol/TokenReceiverPlugin.json"
                    )
                )
