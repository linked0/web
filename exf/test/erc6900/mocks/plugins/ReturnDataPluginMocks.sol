...
import {ManifestFunction, ManifestAssociatedFunctionType, ManifestAssociatedFunction, ManifestExternalCallPermission, PluginManifest} from "@erc6900/interfaces/IPlugin.sol";
...
contract ResultConsumerPlugin is BaseTestPlugin {
    ...
    function checkResultEFPExternal(
        address target,
        bytes32 expected
        ...
        returns (PluginManifest memory)
    {
        ...
        function()
            internal
            pure
            returns (PluginManifest memory) pureManifestGetter;

        function()
            internal
            view
            returns (
                PluginManifest memory
            ) viewManifestGetter = _innerPluginManifest;
