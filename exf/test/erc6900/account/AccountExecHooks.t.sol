...
import {IPlugin, ManifestAssociatedFunctionType, ManifestAssociatedFunction, ManifestExecutionHook, ManifestFunction, PluginManifest} from "@erc6900/interfaces/IPlugin.sol";
...
contract AccountExecHooksTest is OptimizedTest {
    ...
    function _installPlugin1WithHooks(
        ...
        m1.executionHooks.push(
            ManifestExecutionHook(selector, preHook, postHook)
        );
    ...
    function _installPlugin2WithHooksExpectSuccess(
        ...
        if (
            postHook.functionType == ManifestAssociatedFunctionType.DEPENDENCY
        ) {
            m2.dependencyInterfaceIds.push(type(IPlugin).interfaceId);
        }

        m2.executionHooks.push(
            ManifestExecutionHook(selector, preHook, postHook)
        );
