...
import {ManifestAssociatedFunctionType, ManifestExecutionHook, ManifestFunction, PluginManifest} from "@erc6900/interfaces/IPlugin.sol";
...
contract AccountLoupeTest is OptimizedTest {
    ...
    function setUp() public {
        ...
        bytes32 manifestHash = keccak256(
            abi.encode(comprehensivePlugin.pluginManifest())
        );
    ...
    function test_pluginLoupe_getExecutionFunctionConfig_native() public {
        ...
        FunctionReference[]
            memory expectedUserOpValidations = new FunctionReference[](5);
        FunctionReference[]
            memory expectedRuntimeValidations = new FunctionReference[](5);
        ...
        for (uint256 i = 0; i < selectorsToCheck.length; i++) {
            IAccountLoupe.ExecutionFunctionConfig memory config = account1
                .getExecutionFunctionConfig(selectorsToCheck[i]);
    ...
    function test_pluginLoupe_getExecutionFunctionConfig_plugin() public {
        ...
        FunctionReference[]
            memory expectedUserOpValidations = new FunctionReference[](2);
        FunctionReference[]
            memory expectedRuntimeValidations = new FunctionReference[](2);
        ...
        expectedUserOpValidations[0] = FunctionReferenceLib.pack(
            address(comprehensivePlugin),
            uint8(ComprehensivePlugin.FunctionId.USER_OP_VALIDATION)
        );
        expectedRuntimeValidations[0] = FunctionReferenceLib.pack(
            address(comprehensivePlugin),
            uint8(ComprehensivePlugin.FunctionId.RUNTIME_VALIDATION)
        );
    ...
    function test_pluginLoupe_getExecutionHooks() public {
        IAccountLoupe.ExecutionHooks[] memory hooks = account1
            .getExecutionHooks(comprehensivePlugin.foo.selector);
        ...
        assertEq(
            ...
            FunctionReference.unwrap(
                FunctionReferenceLib.pack(
                    address(comprehensivePlugin),
                    uint8(ComprehensivePlugin.FunctionId.PRE_EXECUTION_HOOK)
                )
            )
        );
        assertEq(
            ...
            FunctionReference.unwrap(
                FunctionReferenceLib.pack(
                    address(comprehensivePlugin),
                    uint8(ComprehensivePlugin.FunctionId.POST_EXECUTION_HOOK)
                )
            )
        );
    }
    ...
    function test_pluginLoupe_getHooks_multiple() public {
        ...
        bytes32 manifestHash = keccak256(
            abi.encode(mockPlugin.pluginManifest())
        );
    ...
    function test_pluginLoupe_getPreUserOpValidationHooks() public {
        (FunctionReference[] memory hooks, ) = account1.getPreValidationHooks(
