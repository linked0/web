...
contract ExecuteFromPluginPermissionsTest is OptimizedTest {
    ...
    function setUp() public {
        ...
        bytes32 resultCreatorManifestHash = keccak256(
            abi.encode(resultCreatorPlugin.pluginManifest())
        );
        ...
        bytes32 efpCallerManifestHash = keccak256(
            abi.encode(efpCallerPlugin.pluginManifest())
        );
        account.installPlugin({
        ...
        bytes32 efpCallerAnyExternalManifestHash = keccak256(
            abi.encode(efpCallerPluginAnyExternal.pluginManifest())
        );
        account.installPlugin({
    ...
    function test_getPermissionsTestAddresses() public view {
        ...
        console.log(
            "resultCreatorPlugin address: %s",
            address(resultCreatorPlugin)
        );
        // solhint-enable no-console
    }

    function test_executeFromPluginAllowed() public {
        bytes memory result = EFPCallerPlugin(address(account))
            .useEFPPermissionAllowed();
    ...
    function test_executeFromPluginExternal_Allowed_IndividualSelectors()
        public
    {
        EFPCallerPlugin(address(account)).setNumberCounter1(17);
        uint256 retrievedNumber = EFPCallerPlugin(address(account))
            .getNumberCounter1();

        assertEq(retrievedNumber, 17);
    }

    function test_executeFromPluginExternal_NotAlowed_IndividualSelectors()
        public
    {
        EFPCallerPlugin(address(account)).setNumberCounter1(17);
    ...
    function test_executeFromPluginExternal_Allowed_AnyContract() public {
        ...
        EFPCallerPluginAnyExternal(address(account)).passthroughExecute(
            address(counter1),
            0,
            abi.encodeCall(Counter.setNumber, (17))
        );
        ...
        bytes memory result = EFPCallerPluginAnyExternal(address(account))
            .passthroughExecute(
                address(counter1),
                0,
                abi.encodePacked(bytes4(keccak256("number()")))
        ...
        result = EFPCallerPluginAnyExternal(address(account))
            .passthroughExecute(
                address(counter2),
                0,
                abi.encodePacked(bytes4(keccak256("number()")))
