...
import {RegularResultContract, ResultCreatorPlugin, ResultConsumerPlugin} from "../mocks/plugins/ReturnDataPluginMocks.sol";
...
contract AccountReturnDataTest is OptimizedTest {
    ...
    function setUp() public {
        ...
        bytes32 resultConsumerManifestHash = keccak256(
            abi.encode(resultConsumerPlugin.pluginManifest())
        );
        account.installPlugin({
    ...
    function test_returnData_singular_execute() public {
        bytes memory returnData = account.execute(
            address(regularResultContract),
            0,
            abi.encodeCall(RegularResultContract.foo, ())
        );
    ...
    function test_returnData_execFromPlugin_fallback() public {
        bool result = ResultConsumerPlugin(address(account))
            .checkResultEFPFallback(keccak256("bar"));
