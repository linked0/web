...
abstract contract $GovernorTimelockAccess is GovernorTimelockAccess {
    ...
    function $_setBaseDelaySeconds(uint32 newBaseDelay) external {
        super._setBaseDelaySeconds(newBaseDelay);
    }

    function $_setAccessManagerIgnored(address target,bytes4 selector,bool ignored) external {
        super._setAccessManagerIgnored(target,selector,ignored);
    }

    function $_queueOperations(uint256 proposalId,address[] calldata targets,uint256[] calldata arg2,bytes[] calldata calldatas,bytes32 arg4) external returns (uint48 ret0) {
        (ret0) = super._queueOperations(proposalId,targets,arg2,calldatas,arg4);
        emit return$_queueOperations(ret0);
    }

    function $_executeOperations(uint256 proposalId,address[] calldata targets,uint256[] calldata values,bytes[] calldata calldatas,bytes32 arg4) external {
