...
describe("HashRegistry", function () {
  ...
  async function deploy() {
    const hashTypeA = ethers.solidityPackedKeccak256(
      ["string"],
      ["Hash type A"],
    );
    const hashTypeB = ethers.solidityPackedKeccak256(
      ["string"],
      ["Hash type B"],
    );
    ...
    const roles: Record<string, HardhatEthersSigner> = roleNames.reduce(
      (acc, roleName, index) => {
    ...
    const sortedHashTypeBSigners = Array.from({ length: 2 })
      .map(() => ethers.Wallet.createRandom())
      .sort((a, b) => (BigInt(a.address) > BigInt(b.address) ? 1 : -1));
    const delegates = Array.from({ length: sortedHashTypeASigners.length }).map(
      () => ethers.Wallet.createRandom(),
    );

    const HashRegistry = await ethers.getContractFactory(
      "HashRegistry",
      roles.deployer,
    );
  ...
  describe("constructor", function () {
    context("Owner address is not zero", function () {
      it("constructs", async function () {
        ...
        expect(await hashRegistry.signatureDelegationHashType()).to.equal(
          SIGNATURE_DELEGATION_HASH_TYPE,
        );
      });
    });
    context("Owner address is zero", function () {
      it("reverts", async function () {
        ...
        console.log("Owner address is zero", ethers.ZeroAddress);
        await expect(
          HashRegistry.deploy(ethers.ZeroAddress),
  ...
  describe("transferOwnership", function () {
    it("transfers ownership", async function () {
      ...
      await hashRegistry
        .connect(roles.owner)
        .transferOwnership(roles.randomPerson!.address);
    ...
    context("Sender is the owner", function () {
      context("Hash type is not zero", function () {
          ...
          context("First signer address is not zero", function () {
            context("Signer addresses are in ascending order", function () {
              it("sets signers", async function () {
                ...
                expect(
                  await hashRegistry.hashTypeToSignersHash(hashTypeA),
            ...
            context("Signer addresses are not in ascending order", function () {
              it("reverts", async function () {
                ...
                const unsortedHashTypeASigners = [
                  ...sortedHashTypeASigners.slice(1),
                  sortedHashTypeASigners[0],
                ];
                await expect(
                  hashRegistry.connect(roles.owner).setSigners(
                    hashTypeA,
                ...
                const duplicatedHashTypeASigners = [
                  sortedHashTypeASigners[1],
                  ...sortedHashTypeASigners.slice(1),
                ];
                await expect(
                  hashRegistry.connect(roles.owner).setSigners(
          ...
          context("First signer address is zero", function () {
            it("reverts", async function () {
              ...
              await expect(
                hashRegistry.connect(roles.owner).setSigners(
                  hashTypeA,
                  hashTypeASignersStartingWithZeroAddress.map(
        ...
        context("Signers are empty", function () {
          it("reverts", async function () {
            const { hashTypeA, roles, hashRegistry } =
              await helpers.loadFixture(deploy);
            await expect(
              hashRegistry.connect(roles.owner).setSigners(hashTypeA, []),
            ).to.be.revertedWith("Signers empty");
          });
        });
      });
      context("Hash type is zero", function () {
        it("reverts", async function () {
          const { roles, sortedHashTypeASigners, hashRegistry } =
            await helpers.loadFixture(deploy);
          await expect(
            hashRegistry.connect(roles.owner).setSigners(
              ethers.ZeroHash,
  ...
  describe("setHash", function () {
    context("Sender is the owner", function () {
      it("sets hash", async function () {
        ...
        expect(await hashRegistry.getHashValue(hashTypeA)).to.equal(
          ethers.ZeroHash,
        );
    ...
    context("Sender is not the owner", function () {
      it("reverts", async function () {
        ...
        const hash = ethers.hexlify(ethers.randomBytes(32));
        await expect(
          hashRegistry.connect(roles.randomPerson).setHash(hashTypeA, hash),
        ).to.be.revertedWithCustomError(
          hashRegistry,
          "OwnableUnauthorizedAccount",
        );
            ...
            context("No delegation signature is used", function () {
              context("All signatures match", function () {
                it("registers hash", async function () {
                  ...
                  const signatures = await signHash(
                    sortedHashTypeASigners,
                    hashTypeA,
                    hash,
                    timestamp,
                  ...
                  await expect(
                    hashRegistry
                      ...
                      .registerHash(hashTypeA, hash, timestamp, signatures),
                  )
                        ...
                        context(
                          ...
                          function () {
                            it("registers hash", async function () {
                              ...
                              const delegateSignatures = await signHash(
                                delegates,
                                hashTypeA,
                                hash,
                                timestamp,
                              ...
                              const hashBefore =
                                await hashRegistry.hashes(hashTypeA);
                              expect(hashBefore.value).to.equal(
                                ethers.ZeroHash,
                              );
                              expect(hashBefore.timestamp).to.equal(0);
                              expect(
                                await hashRegistry.getHashValue(hashTypeA),
                              ).to.equal(ethers.ZeroHash);
                              await expect(
                                hashRegistry
                                  .connect(roles.randomPerson)
                                  .registerHash(hashTypeA, hash, timestamp, [
                                    ...
                                    ethers.AbiCoder.defaultAbiCoder().encode(
                                      ...
                                      [
                                        delegationEndTimestamp,
                                        delegationSignatures[2],
                                        delegateSignatures[2],
                                      ],
                                    ),
                                  ]),
                              )
    ...
    context("Hash value is not zero", function () {
      it("reverts", async function () {
        ...
        await expect(
          hashRegistry
            .connect(roles.randomPerson)
            .registerHash(hashTypeA, ethers.ZeroHash, timestamp, signatures),
