...
import {AdvancedOrder, OfferItem, OrderParameters, ConsiderationItem, OrderComponents, FulfillmentComponent, CriteriaResolver} from "seaport-types/src/lib/ConsiderationStructs.sol";
...
contract FulfillAvailableAdvancedOrder is BaseOrderTest {
    ...
    public
  {
    for (uint256 i; i < 4; ++i) {
      ...
      test(
        this.fulfillAvailableAdvancedOrdersAggregateMissingOfferItemAmounts,
        Context(consideration, empty, ItemType(i))
      );
      test(
        this.fulfillAvailableAdvancedOrdersAggregateMissingOfferItemAmounts,
  ...
  function fulfillAvailableAdvancedOrdersAggregateMissingConsiderationItemAmounts(
    ...
    if (context.itemType == ItemType.ERC721) {
      addConsiderationItem(alice, ItemType.ERC721, address(test721_1), 0, 0, 0);
    } else {
  ...
  function fulfillSingleOrderViaFulfillAvailableAdvancedOrdersEthToErc1155(
    ...
    considerationItems.push(
      ConsiderationItem(ItemType.NATIVE, address(0), 0, 10, 10, payable(alice))
    );
    considerationItems.push(
      ConsiderationItem(
        ItemType.NATIVE,
    ...
    considerationItems.push(
      ConsiderationItem(ItemType.NATIVE, address(0), 0, 10, 10, payable(cal))
    );
