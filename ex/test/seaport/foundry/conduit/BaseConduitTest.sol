...
import {ConduitTransfer, ConduitItemType, ConduitBatch1155Transfer} from "seaport-types/src/conduit/lib/ConduitStructs.sol";
  ...
  ERC721Recipient
{
  ...
  function isErc1155Receiver(address to) internal returns (bool) {
    if (to == address(0)) {
      ...
      return
        ...
        keccak256(abi.encode(ERC1155TokenReceiver.onERC1155Received.selector));
    } else {
      return true;
    }
  }
  ...
  function deployTokenAndCreateConduitTransfers(
    ...
    ConduitItemType itemType = ConduitItemType((intermediate.itemType % 3) + 1);
    address token;
    if (itemType == ConduitItemType.ERC20) {
  ...
  function mintTokensAndSetTokenApprovalsForConduit(
    ...
    for (uint256 i = 0; i < batchTransfers.length; ++i) {
      ...
      for (uint256 n = 0; n < batchTransfer.ids.length; ++n) {
        erc1155.mint(from, batchTransfer.ids[n], batchTransfer.amounts[n]);
      }
      vm.startPrank(from);
      erc1155.setApprovalForAll(address(conduit), true);
      erc1155.setApprovalForAll(address(referenceConduit), true);
