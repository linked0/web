...
import {AdvancedOrder, OrderParameters, OrderComponents, CriteriaResolver} from "seaport-types/src/lib/ConsiderationStructs.sol";
...
contract FulfillAdvancedOrder is BaseOrderTest {
  ...
  function testNoNativeOffersFulfillAdvanced(uint8[8] memory itemTypes) public {
    uint256 tokenId;
    for (uint256 i; i < 8; i++) {
  ...
  function testAdvancedPartialAscendingOfferAmount1155(
    ...
    test(
      ...
      Context(referenceConsideration, args, tokenAmount, warpAmount % 1000)
    );
    test(
      this.advancedPartialAscendingOfferAmount1155,
  ...
  function advancedPartialAscendingConsiderationAmount1155(
    ...
    test1155_1.mint(alice, context.args.tokenId, context.tokenAmount.mul(1000));
  ...
  function revertPartialFulfillInexactFractionEthTo1155(
    ...
    context.consideration.fulfillAdvancedOrder{value: 100}(
      AdvancedOrder(baseOrderParameters, context.args.numer, 69, signature, ""),
      new CriteriaResolver[](0),
      bytes32(0),
      address(0)
    );
