...
import {OfferItem, ConsiderationItem, AdvancedOrder, CriteriaResolver, OrderComponents, FulfillmentComponent} from "seaport-types/src/lib/ConsiderationStructs.sol";
...
contract StatefulOffererTest is BaseOrderTest {
  ...
  function execMatchAdvancedOrders(Context memory context) external stateless {
    offerer = new StatefulRatifierOfferer(
      address(context.consideration),
      ERC20Interface(address(token1)),
      ERC721Interface(address(test721_1)),
