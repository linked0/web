...
import {AdvancedOrder, BasicOrderParameters, CriteriaResolver, Execution, Fulfillment, FulfillmentComponent, OrderParameters} from "seaport-sol/src/SeaportStructs.sol";

import {ItemType, OrderType, Side} from "seaport-sol/src/SeaportEnums.sol";

import {BroadOrderType, OrderStatusEnum, SignatureMethod, UnavailableReason} from "seaport-sol/src/SpaceEnums.sol";
...
import {ErrorsAndWarnings, SeaportValidatorInterface} from "../../../../contracts/helpers/order-validator/SeaportValidator.sol";

import {SeaportNavigatorInterface} from "../../../../contracts/helpers/navigator/SeaportNavigator.sol";
...
library FuzzTestContextLib {
  ...
  function withOrders(
    ...
    if (context.executionState.orderDetails.length == 0) {
      context.executionState.orderDetails = new OrderDetails[](orders.length);
  ...
  function withPreExecOrderStatuses(
    ...
    for (uint256 i = 0; i < context.executionState.orders.length; i++) {
      if (space.orders[i].orderType == BroadOrderType.CONTRACT) {
        ...
        context.executionState.preExecOrderStatuses[i] = OrderStatusEnum
          .FULFILLED;
      } else if (space.orders[i].signatureMethod == SignatureMethod.VALIDATE) {
        ...
        if (
          ...
          revert("FuzzTestContextLib: bad location for generate order failure");
        }
        ...
        context.executionState.preExecOrderStatuses[i] = OrderStatusEnum(
          uint8(bound(prng.next(), 0, upperBound))
        );
      }
    }
  ...
  function _copyFulfillmentComponents(
    ...
    FulfillmentComponent[][] memory outerCopy = new FulfillmentComponent[][](
      fulfillmentComponents.length
    );
    for (uint256 i = 0; i < fulfillmentComponents.length; i++) {
      FulfillmentComponent[] memory innerCopy = new FulfillmentComponent[](
