...
import {ConsiderationItemLib, OfferItemLib, OrderParametersLib} from "seaport-sol/src/SeaportSol.sol";

import {MatchFulfillmentLib, ProcessComponentParams} from "seaport-sol/src/fulfillments/match/MatchFulfillmentLib.sol";
...
import {ConsiderationItem, Fulfillment, FulfillmentComponent, OfferItem, OrderParameters} from "seaport-sol/src/SeaportStructs.sol";
...
contract MatchFulfillmentLibTest is Test {
  ...
  function testConsolidateComponents(uint240[10] memory amounts) public {
    ...
    for (uint256 i = 0; i < 10; i++) {
      MatchComponent memory temp = MatchComponentType.createMatchComponent(
        amounts[i],
        0,
        0
      );
    ...
    // call function
    MatchFulfillmentLib.consolidateComponents(_components, _components.length);
  ...
  function testProcessOfferComponent() public {
    ...
    assertEq(params.offerItemIndex, 1, "processOfferComponent offerItemIndex");
    assertEq(
      offer[0].getAmount(),
      0,
    ...
    offerFulfillmentComponents = MatchArrays.allocateFulfillmentComponents(2);
    ...
    MatchFulfillmentLib.processOfferComponent(offer, consideration, params);
    assertEq(params.offerItemIndex, 0, "processOfferComponent offerItemIndex");
    assertEq(
      params.offerFulfillmentComponents.length,
      1,
      "offerFulfillmentComponents length"
    ...
    MatchFulfillmentLib.processOfferComponent(offer, consideration, params);
    assertEq(consideration[0].getAmount(), 0, "consideration[0].getAmount() 4");
  ...
  function assertEq(
    ...
    FulfillmentComponent memory leftComponent = left.toFulfillmentComponent();
    FulfillmentComponent memory rightComponent = right.toFulfillmentComponent();
