...
import {AdvancedOrderLib, ConsiderationItemLib, MatchComponent, MatchComponentType, OfferItemLib, OrderLib, OrderParametersLib} from "seaport-sol/src/SeaportSol.sol";

import {AdvancedOrder, ConsiderationItem, CriteriaResolver, Fulfillment, OfferItem, Order, OrderComponents, OrderParameters, ReceivedItem, SpentItem} from "seaport-sol/src/SeaportStructs.sol";
...
import {Amount, BasicOrderCategory, BroadOrderType, Caller, ConduitChoice, ContractOrderRebate, Criteria, EOASignature, ExtraData, FulfillmentRecipient, Offerer, Recipient, SignatureMethod, Time, Tips, TokenIndex, UnavailableReason, Zone, ZoneHash} from "seaport-sol/src/SpaceEnums.sol";
...
import {AdvancedOrdersSpace, ConsiderationItemSpace, OfferItemSpace, OrderComponentsSpace} from "seaport-sol/src/StructSpace.sol";

import {EIP712MerkleTree} from "../../utils/EIP712MerkleTree.sol";

import {FuzzGeneratorContext, TestConduit} from "./FuzzGeneratorContextLib.sol";
...
import {AggregationStrategy, FulfillAvailableStrategy, MatchStrategy, FulfillmentGeneratorLib, FulfillmentStrategy, DefaultFulfillmentGeneratorLib} from "seaport-sol/src/fulfillments/lib/FulfillmentLib.sol";
    ...
    for (uint256 i; i < totalOrders; ++i) {
      ...
      if (components[i].orderType == BroadOrderType.CONTRACT) {
        ...
        if (
          ...
          components[i].unavailableReason == UnavailableReason.CANCELLED
        ) {
    ...
    if (!isBasic) {
      for (uint256 i; i < len; ++i) {
        consideration[i] = ConsiderationItemSpace({
          ...
          tokenIndex: TokenIndex(context.randEnum(0, 2)),
          criteria: Criteria(context.randEnum(0, 1)),
          amount: atLeastOne ? Amount.FIXED : Amount(context.randEnum(0, 2)),
...
library AdvancedOrdersSpaceGenerator {
    ...
    for (uint256 i = 0; i < orders.length; ++i) {
      ...
      if (orderParams.orderType == OrderType.CONTRACT) {
        ...
        for (uint256 j = 0; j < orderParams.offer.length; ++j) {
          ...
          if (uint256(item.itemType) > 3 && item.identifierOrCriteria == 0) {
        ...
        for (uint256 j = 0; j < orderParams.consideration.length; ++j) {
          ...
          if (item.startAmount != 0) {
            order.parameters.consideration[j].endAmount = (item.startAmount);
          } else {
            order.parameters.consideration[j].startAmount = (item.endAmount);
          }
          ...
          if (uint256(item.itemType) > 3 && item.identifierOrCriteria == 0) {
            ...
            bytes32 itemHash = keccak256(
              abi.encodePacked(uint256(i), uint256(j), Side.CONSIDERATION)
            );
            order.parameters.consideration[j].identifierOrCriteria = context
              .testHelpers
              .criteriaResolverHelper()
              .wildcardIdentifierForGivenItemHash(itemHash);
          }
    ...
    for (uint256 i = 0; i < orders.length; ++i) {
      ...
      for (uint256 j = 0; j < order.offer.length; ++j) {
        ...
        if (item.itemType == ItemType.NATIVE) {
          ...
          orders[i].parameters.offer[j] = space.orders[i].offer[j].generate(
            context,
            true,
            i,
            j
  ...
  function _ensureAllAvailable(AdvancedOrdersSpace memory space) internal pure {
  ...
  function _buildOrders(
    ...
    for (uint256 i; i < orders.length; ++i) {
      ...
      orders[i] = OrderLib
        ...
        .toAdvancedOrder({numerator: 1, denominator: 1, extraData: bytes("")})
  ...
  function _adjustUnavailable(
    ...
    for (uint256 i = 0; i < orders.length; ++i) {
      _adjustUnavailable(orders[i], space.orders[i].unavailableReason, context);
    }
  }
  ...
  function _adjustUnavailable(
    ...
    if (reason == UnavailableReason.EXPIRED) {
      ...
      parameters = parameters.withGeneratedTime(Time.STARTS_IN_FUTURE, context);
    }
  }
  ...
  function _handleInsertIfAllFilterable(
    ...
    if (allFilterable) {
      ...
      uint256 orderInsertionIndex = context.randRange(0, orders.length - 1);
      for (; orderInsertionIndex < orders.length * 2; ++orderInsertionIndex) {
    ...
    for (uint256 i = 0; i < orders.length; ++i) {
      ...
      if (order.parameters.orderType == OrderType.CONTRACT) {
        uint256 contractOffererSpecificContractNonce = context
          .contractOffererNonce + uint256(uint160(order.parameters.offerer));
...
library ConduitGenerator {
  function generate(
    ...
    if (conduit == ConduitChoice.NONE) {
      return TestConduit({key: bytes32(0), addr: address(context.seaport)});
...
library ExtraDataGenerator {
  ...
  function withGeneratedExtraData(
    ...
    if (extraData == ExtraData.NONE) {
      ...
      return order.withExtraData(generateRandomBytesArray(context, 1, 4096));
    } else {
  ...
  function withGeneratedSignature(
    ...
    if (method == SignatureMethod.EOA) {
      ...
      if (eoaSignatureType == EOASignature.STANDARD) {
        ...
        (infra.v, infra.r, infra.s) = context.vm.sign(offererKey, infra.digest);
        signature = abi.encodePacked(infra.r, infra.s, infra.v);

        _checkSig(infra.digest, infra.v, infra.r, infra.s, offerer, context);
...
library TokenIndexGenerator {
  function generate(
    ...
    if (itemType == ItemType.ERC20) {
      ...
      itemType == ItemType.ERC721 || itemType == ItemType.ERC721_WITH_CRITERIA
    ) {
      return address(context.erc721s[i]);
    } else if (
      itemType == ItemType.ERC1155 || itemType == ItemType.ERC1155_WITH_CRITERIA
    ) {
