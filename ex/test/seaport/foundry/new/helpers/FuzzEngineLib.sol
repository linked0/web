...
import {AdvancedOrderLib, MatchComponent, OrderComponentsLib, OrderLib, OrderParametersLib} from "seaport-sol/src/SeaportSol.sol";

import {AdvancedOrder, ConsiderationItem, Execution, OfferItem, Order, OrderComponents, OrderParameters, SpentItem, ReceivedItem} from "seaport-sol/src/SeaportStructs.sol";
...
import {_locateCurrentAmount, Family, FuzzHelpers, Structure} from "./FuzzHelpers.sol";

import {FuzzTestContext} from "./FuzzTestContextLib.sol";

import {FuzzDerivers} from "./FuzzDerivers.sol";

import {DefaultFulfillmentGeneratorLib} from "seaport-sol/src/fulfillments/lib/FulfillmentLib.sol";
...
library FuzzEngineLib {
  ...
  function actions(
    ...
    if (hasUnavailable) {
      ...
      if (structure == Structure.ADVANCED) {
        ...
        selectors[0] = context.seaport.fulfillAvailableAdvancedOrders.selector;
        return selectors;
      } else {
        bytes4[] memory selectors = new bytes4[](2);
        selectors[0] = context.seaport.fulfillAvailableOrders.selector;
        selectors[1] = context.seaport.fulfillAvailableAdvancedOrders.selector;
        return selectors;
      }
    }
    ...
    if (family == Family.SINGLE && !containsOrderThatDemandsMatch) {
      if (structure == Structure.BASIC) {
        ...
        selectors[5] = context.seaport.fulfillAvailableAdvancedOrders.selector;
        return selectors;
      }
      ...
      if (structure == Structure.STANDARD) {
        ...
        selectors[3] = context.seaport.fulfillAvailableAdvancedOrders.selector;
        return selectors;
      }
    ...
    bool cannotMatch = (context.executionState.hasRemainders || hasUnavailable);
    ...
    for (uint256 i = 0; i < orders.length; ++i) {
      ...
      for (uint256 j = 0; j < order.offer.length; ++j) {
        ...
        for (uint256 k = 0; k < orders.length; ++k) {
          ...
          for (uint256 l = 0; l < comparisonOrder.consideration.length; ++l) {
  ...
  function getMinimumNativeTokensToSupply(
    ...
    if (
      ...
      _action == context.seaport.fulfillBasicOrder_efficient_6GL6yc.selector
    ) {
