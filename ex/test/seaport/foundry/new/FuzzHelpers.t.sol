...
import {AdvancedOrderLib, ConsiderationItemLib, FulfillmentComponentLib, FulfillmentLib, OfferItemLib, OrderComponentsLib, OrderLib, OrderParametersLib} from "seaport-sol/src/SeaportSol.sol";
...
import {BasicOrderType, ItemType, OrderType} from "seaport-sol/src/SeaportEnums.sol";

import {BaseOrderTest} from "./BaseOrderTest.sol";

import {AdvancedOrder, Family, FuzzHelpers, State, Structure, Type} from "./helpers/FuzzHelpers.sol";
...
contract FuzzHelpersTest is BaseOrderTest {
  ...
  function test_getStructure_Advanced_OfferERC721Criteria() public {
    ...
    offer[0] = OfferItemLib.empty().withItemType(ItemType.ERC721_WITH_CRITERIA);
  ...
  function test_getBasicOrderType_ETH_TO_ERC721_FULL_OPEN() public {
    ...
    assertEq(order.getBasicOrderType(), BasicOrderType.ETH_TO_ERC721_FULL_OPEN);
  }
   ...
   */
  function test_getBasicOrderType_ERC20_TO_ERC721_PARTIAL_RESTRICTED() public {
    AdvancedOrder memory order = _createOrder(
      ItemType.ERC721,
      ItemType.ERC20,
      OrderType.PARTIAL_RESTRICTED
   ...
   *      supported, only offerer or zone can execute.
   */
  function test_getBasicOrderType_ERC20_TO_ERC1155_PARTIAL_RESTRICTED() public {
    AdvancedOrder memory order = _createOrder(
      ItemType.ERC1155,
      ItemType.ERC20,
      OrderType.PARTIAL_RESTRICTED
   ...
   *      supported, only offerer or zone can execute.
   */
  function test_getBasicOrderType_ERC721_TO_ERC20_PARTIAL_RESTRICTED() public {
    AdvancedOrder memory order = _createOrder(
      ItemType.ERC20,
      ItemType.ERC721,
      OrderType.PARTIAL_RESTRICTED
   ...
   *      supported, only offerer or zone can execute.
   */
  function test_getBasicOrderType_ERC1155_TO_ERC20_PARTIAL_RESTRICTED() public {
    AdvancedOrder memory order = _createOrder(
      ItemType.ERC20,
      ItemType.ERC1155,
      OrderType.PARTIAL_RESTRICTED
  ...
  function test_getBasicOrderTypeEligibility_failure_criteria() public {
    ...
    order.parameters.consideration[0].itemType = ItemType.ERC1155_WITH_CRITERIA;
  ...
  function test_getBasicOrderTypeEligibility_failure_offerItemLength() public {
    AdvancedOrder memory order = _createOrder(
      ItemType.ERC20,
      ItemType.ERC1155,
      OrderType.PARTIAL_RESTRICTED
