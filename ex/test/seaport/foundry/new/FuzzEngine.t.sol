...
import {AdvancedOrderLib, ConsiderationItemLib, FulfillmentComponentLib, FulfillmentLib, OfferItemLib, OrderComponentsLib, OrderLib, OrderParametersLib, SeaportArrays, ZoneParametersLib} from "seaport-sol/src/SeaportSol.sol";

import {ConsiderationItem, CriteriaResolver, Fulfillment, FulfillmentComponent, ItemType, OfferItem, Order, OrderComponents, OrderParameters, OrderType} from "seaport-sol/src/SeaportStructs.sol";
...
import {HashValidationZoneOfferer} from "../../../contracts/test/HashValidationZoneOfferer.sol";

import {TestCalldataHashContractOfferer} from "../../../contracts/test/TestCalldataHashContractOfferer.sol";

import {FuzzEngine, FuzzEngineLib, FuzzParams, FuzzTestContext, FuzzTestContextLib} from "./helpers/FuzzEngine.sol";
...
contract FuzzEngineTest is FuzzEngine {
  ...
  function test_actions_Single_Standard() public {
    ...
    FuzzTestContext memory context = FuzzTestContextLib
      .from({orders: orders, seaport: getSeaport(), caller: address(this)})
      .withFuzzParams(
        FuzzParams({
          seed: 0,
          totalOrders: 0,
  ...
  function test_action_Single_Standard() public {
    ...
    context = FuzzTestContextLib
      ...
      .withMaximumFulfilled(1);
    assertEq(context.action(), SeaportInterface.fulfillAdvancedOrder.selector);
  }
  ...
  function test_action_Combined() public {
    ...
    context = FuzzTestContextLib
      ...
      .withMaximumFulfilled(2);
    assertEq(context.action(), SeaportInterface.matchAdvancedOrders.selector);
  ...
  function _setUpBasicOrder() internal returns (AdvancedOrder[] memory) {
    ...
    Order memory order = OrderLib
      .fromDefault(STANDARD)
      .withParameters(
        orderComponents.toOrderParameters().withOrderType(OrderType.FULL_OPEN)
      )
      .withSignature(signature);
  ...
  function test_exec_Combined_FulfillAvailable() public {
    ...
    ConsiderationItem[] memory considerationItems1 = new ConsiderationItem[](1);
    ConsiderationItem memory considerationItem = ConsiderationItemLib
      .empty()
      .withRecipient(offerer1.addr)
      .withItemType(ItemType.ERC721)
    ...
    ConsiderationItem[] memory considerationItems2 = new ConsiderationItem[](1);
    ...
    assertEq(context.returnValues.executions[0].item.itemType, ItemType.ERC20);
    assertEq(context.returnValues.executions[0].item.token, address(erc20s[0]));
    assertEq(context.returnValues.executions[0].item.identifier, 0);
    assertEq(context.returnValues.executions[0].item.amount, 1);
    assertEq(context.returnValues.executions[0].item.recipient, address(this));

    assertEq(context.returnValues.executions[1].item.itemType, ItemType.ERC20);
    assertEq(context.returnValues.executions[1].item.token, address(erc20s[0]));
    assertEq(context.returnValues.executions[1].item.identifier, 0);
    assertEq(context.returnValues.executions[1].item.amount, 1);
    assertEq(context.returnValues.executions[1].item.recipient, address(this));

    assertEq(context.returnValues.executions[2].item.itemType, ItemType.ERC721);
    assertEq(
    ...
    assertEq(context.returnValues.executions[2].item.recipient, offerer1.addr);

    assertEq(context.returnValues.executions[3].item.itemType, ItemType.ERC721);
    assertEq(
    ...
    assertEq(context.returnValues.executions[3].item.recipient, offerer1.addr);
  ...
  function test_exec_Combined_FulfillAvailableAdvanced() public {
    ...
    {
      OfferItem[] memory offerItems = new OfferItem[](1);
      ConsiderationItem[] memory considerationItems1 = new ConsiderationItem[](
        1
      );
      ConsiderationItem[] memory considerationItems2 = new ConsiderationItem[](
        1
      );
      {
        // Offer ERC20
  ...
  function test_exec_Combined_matchAdvancedOrders() public {
    ...
    FuzzTestContext memory context = FuzzTestContextLib
      .from({orders: advancedOrders, seaport: seaport, caller: offerer1.addr})
      .withFuzzParams(
        FuzzParams({
          seed: 3,
          totalOrders: 0,
  ...
  function xtest_exec_Combined_Cancel() public {
    ...
    FuzzTestContext memory context = FuzzTestContextLib
      .from({orders: orders, seaport: getSeaport(), caller: offerer1.addr})
      .withFuzzParams(
        FuzzParams({
          seed: 4,
          totalOrders: 0,
