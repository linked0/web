...
async function main() {
  ...
  const create2Factory = await ethers.getContractAt(
    "Create2Factory",
    process.env.CREATE2FACTORY_ADDRESS,
  );
  ...
  initCodeStr =
    "6080604052348015600f57600080fd5b50603f80601d6000396000f3fe6080604052600080fdfea26469706673582212205c308266d3e1e69ef40d52cec3a293b9f93f5962c0b39b8e9bf06874cf47522e64736f6c63430008130033";
...
function getDeployedAddress(initCode: string, salt: number): string {
  ...
  return (
    "0x" +
    keccak256(
      concat(["0xff", FACTORY_ADDRESS, saltBytes32, keccak256(initCode)]),
    ).slice(-40)
  );
}
