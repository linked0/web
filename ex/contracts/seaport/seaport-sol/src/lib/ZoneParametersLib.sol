...
import {AdvancedOrder, ConsiderationItem, CriteriaResolver, OfferItem, Order, OrderComponents, OrderParameters, SpentItem, ReceivedItem, ZoneParameters, CriteriaResolver} from "../../../seaport-types/src/lib/ConsiderationStructs.sol";
  ...
  function _isUnavailable(
    ...
    if (order.orderType == OrderType.CONTRACT) {
      isRevertingContractOrder =
        FailingContractOfferer(order.offerer).failureReasons(orderHash) != 0;
    }
