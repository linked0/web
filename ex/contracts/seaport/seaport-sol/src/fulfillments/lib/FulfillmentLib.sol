...
library FulfillmentGeneratorLib {
    ...
    )
  {
    ...
    for (uint256 i = 0; i < matchDetails.items.length; ++i) {
      ...
      if (context.totalConsiderationAmount > context.totalOfferAmount) {
        ...
        if (
          item.orderIndex > type(uint8).max || item.itemIndex > type(uint8).max
        ) {
    ...
    )
  {
    ...
    (considerationFulfillments, considerationCategories) = fulfillmentMethod(
...
library FulfillmentPrepLib {
  ...
  {
    ItemReferenceGroupLib.ItemReferenceGroup[] memory groups = itemReferences
...
library FulfillAvailableReferenceGroupLib {
  ...
  function getFulfillAvailableDetails(
    ...
    (DualFulfillmentItems memory items, uint256 totalItems) = FulfillmentPrepLib
      .getDualFulfillmentItems(group.offerGroups, group.considerationGroups);
...
library ItemReferenceGroupLib {
    ...
    )
  {
    ...
    for (uint256 i = 0; i < groups.length; ++i) {
      ...
      ItemReferenceLib.ItemReference memory firstReference = group.references[
        0
      ];
...
library HashAllocatorLib {
  ...
  ) internal pure returns (ItemReferenceGroupLib.ItemReferenceGroup[] memory) {
