...
import {FulfillAvailableHelperStorageLayout, FulfillmentHelperCounterLayout, AggregatableOffer, AggregatableConsideration} from "../lib/Structs.sol";
...
contract FulfillAvailableHelper {
    ...
    )
  {
    ...
    for (uint256 i = 0; i < orderParameters.length; i++) {
      ...
      for (uint256 j; j < parameters.consideration.length; j++) {
        consideration[considerationIndex] = SeaportArrays.FulfillmentComponents(
          FulfillmentComponent({orderIndex: i, itemIndex: j})
        );
        ++considerationIndex;
      }
    ...
    )
  {
    ...
    FulfillAvailableHelperStorageLayout storage layout = FulfillAvailableLayout
      .getStorageLayout();
    ...
    for (uint256 i; i < layout.considerationEnumeration.length; ++i) {
      AggregatableConsideration memory token = layout.considerationEnumeration[
        i
      ];
  ...
  function preProcessOffer(
    ...
    for (uint256 j; j < offer.length; ++j) {
      ...
      layout
      .offerMap[aggregatableOffer.contractAddress][aggregatableOffer.tokenId][
        aggregatableOffer.offerer
      ][aggregatableOffer.conduitKey].push(component);
    }
  }
  ...
  function preProcessConsideration(
    ...
    for (uint256 j; j < consideration.length; ++j) {
      ...
      // if it does not exist in the map, add it to our enumeration
      if (
        !FulfillAvailableLayout.aggregatableConsiderationExists(token, layout)
      ) {
