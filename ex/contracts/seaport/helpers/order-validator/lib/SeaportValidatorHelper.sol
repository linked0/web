...
import {Order, OrderParameters, BasicOrderParameters, OfferItem, ConsiderationItem, Schema, ZoneParameters} from "../../../seaport-types/src/lib/ConsiderationStructs.sol";
...
import {ZoneInterface} from "../../../seaport-types/src/interfaces/ZoneInterface.sol";
import {ERC20Interface, ERC721Interface, ERC1155Interface} from "../../../seaport-types/src/interfaces/AbridgedTokenInterfaces.sol";
...
contract SeaportValidatorHelper is Murky {
  ...
  function validateConsiderationItemParameters(
    ...
    ConsiderationItem memory considerationItem = orderParameters.consideration[
      considerationItemIndex
    ];

    // Check if startAmount and endAmount are zero
    if (
      considerationItem.startAmount == 0 && considerationItem.endAmount == 0
    ) {
    ...
    if (
      ...
      (uint256 maxAmount, uint256 minAmount) = considerationItem.startAmount >
    ...
    if (considerationItem.itemType == ItemType.ERC721) {
      ...
      if (
        considerationItem.startAmount != 1 || considerationItem.endAmount != 1
      ) {
    ...
    )
  {
    ...
    if (
      ...
      if (
        orderParameters.consideration.length - 1 < creatorFeeConsiderationIndex
      ) {
        errorsAndWarnings.addError(CreatorFeeIssue.Missing.parseInt());
        return (0, errorsAndWarnings);
      }

      ConsiderationItem memory creatorFeeItem = orderParameters.consideration[
    ...
    )
  {
    ...
    if (address(creatorFeeEngine) != address(0)) {
      ...
      try
        creatorFeeEngine.getRoyaltyView(token, tokenId, transactionAmountStart)
      returns (
      ...
      if (recipient != address(0)) {
        // Creator fee engine may revert if no creator fees are present.
        try
          creatorFeeEngine.getRoyaltyView(token, tokenId, transactionAmountEnd)
        returns (
