...
import {IssueParser, ValidationConfiguration, TimeIssue, StatusIssue, OfferIssue, ContractOffererIssue, ConsiderationIssue, PrimaryFeeIssue, ERC721Issue, ERC1155Issue, ERC20Issue, NativeIssue, ZoneIssue, ConduitIssue, CreatorFeeIssue, SignatureIssue, GenericIssue, ConsiderationItemConfiguration} from "./lib/SeaportValidatorTypes.sol";
  ...
  ConsiderationTypeHashes
{
  ...
  function getApprovalAddress(
    bytes32 conduitKey,
    address seaportAddress
  ) public view returns (address, ErrorsAndWarnings memory errorsAndWarnings) {
    errorsAndWarnings = ErrorsAndWarnings(new uint16[](0), new uint16[](0));
  ...
  function validateSignature(
    ...
    uint256 currentCounter = ConsiderationInterface(seaportAddress).getCounter(
  ...
  function validateOfferItemApprovalAndBalance(
    ...
    (address approvalAddress, ErrorsAndWarnings memory ew) = getApprovalAddress(
