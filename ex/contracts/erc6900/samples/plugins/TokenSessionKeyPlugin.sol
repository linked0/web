...
import {ManifestFunction, ManifestAssociatedFunctionType, ManifestAssociatedFunction, PluginManifest, PluginMetadata, SelectorPermission, ManifestExternalCallPermission} from "../../interfaces/IPlugin.sol";
...
contract TokenSessionKeyPlugin is BasePlugin, ITokenSessionKeyPlugin {
  ...
  address public constant TARGET_ERC20_CONTRACT =
  ...
  function transferFromSessionKey(
    address target,
    address from,
    address to,
    uint256 amount
    ...
    returnData = IPluginExecutor(msg.sender).executeFromPluginExternal(
      target,
      0,
      data
    );
    ...
    returns (PluginManifest memory)
  {
    ...
    ManifestFunction
      memory tempOwnerUserOpValidationFunction = ManifestFunction({
        functionType: ManifestAssociatedFunctionType.DEPENDENCY,
        functionId: 0, // Unused
        dependencyIndex: 0 // Used as first index
      });
    ManifestFunction
      memory tempOwnerRuntimeValidationFunction = ManifestFunction({
        functionType: ManifestAssociatedFunctionType.DEPENDENCY,
        functionId: 0, // Unused
    ...
    manifest.dependencyInterfaceIds[0] = type(IModularSessionKeyPlugin)
      .interfaceId;
    manifest.dependencyInterfaceIds[1] = type(IModularSessionKeyPlugin)
      .interfaceId;
  ...
  function supportsInterface(
    ...
    return
      interfaceId == type(ITokenSessionKeyPlugin).interfaceId ||
