...
import {AggregatableConsideration, ProcessComponentParams, AggregatableOfferer, OrderDetails, MatchFulfillmentStorageLayout} from "../lib/Structs.sol";
import {MatchComponent, MatchComponentType} from "../../lib/types/MatchComponentType.sol";
import {FulfillmentComponent, Fulfillment, Order, AdvancedOrder, OrderParameters, SpentItem, ReceivedItem, CriteriaResolver} from "../../SeaportStructs.sol";
...
contract MatchFulfillmentHelper is AmountDeriverHelper {
    ...
    )
  {
    ...
    for (uint256 i; i < considerationLength; ++i) {
      ...
      AggregatableConsideration storage token = layout.considerationEnumeration[
        i
      ];
      ...
      for (uint256 j; j < offererEnumeration.length; ++j) {
        ...
        Fulfillment memory fulfillment = MatchFulfillmentLib.createFulfillment(
          offerComponents,
  ...
  function preProcessSpentItems(
    ...
    for (uint256 j; j < offer.length; ++j) {
      ...
      if (
        ...
        layout.tokenToOffererEnumeration[item.token][item.identifier].push(
          aggregatableOfferer
        );
      }
      // update aggregatable mapping array with this component
      layout.offerMap[item.token][item.identifier][offerer][conduitKey].push(
        component
      );
    }
  }
  ...
  function preProcessSpentItems(
    ...
    for (uint256 j; j < consideration.length; ++j) {
      ...
      layout
      .considerationMap[token.recipient][token.contractAddress][token.tokenId]
        .push(component);
    }
  }
    ...
    returns (MatchComponent[] memory remainingConsiderationComponents)
  {
    ...
    for (uint256 j; j < consideration.length; ++j) {
      ...
      remainingConsiderationComponents = MatchArrays.extend(
        remainingConsiderationComponents,
        layout.considerationMap[item.recipient][item.token][item.identifier]
      );
    }
  }
}
