...
import {AdditionalRecipient, AdvancedOrder, BasicOrderParameters, ConsiderationItem, CriteriaResolver, Execution, Fulfillment, FulfillmentComponent, OfferItem, OrderParameters, ReceivedItem} from "seaport-sol/src/SeaportStructs.sol";

import {BasicOrderType, ItemType, OrderType, Side} from "seaport-sol/src/SeaportEnums.sol";

import {Result} from "./FuzzHelpers.sol";

import {FuzzParams, FuzzTestContext, ReturnValues} from "./FuzzTestContextLib.sol";

import {ERC1155AccountDump, ERC1155TokenDump, ERC20TokenDump, ERC721TokenDump, ExpectedBalancesDump, NativeAccountDump} from "./ExpectedBalances.sol";

import {withLabel} from "./Labeler.sol";

import {ErrorsAndWarnings} from "../../../../contracts/helpers/order-validator/SeaportValidator.sol";
...
library Searializer {
  ...
  function tojsonConsiderationItem(
    ...
    string memory finalJson = tojsonAddress(obj, "recipient", value.recipient);
    return vm.serializeString(objectKey, valueKey, finalJson);
  }
  ...
  function tojsonOrderParameters(
    ...
    tojsonDynArrayConsiderationItem(obj, "consideration", value.consideration);
  ...
  function tojsonFulfillmentComponent(
    ...
    string memory obj = string.concat(objectKey, valueKey);
    tojsonUint256(obj, "orderIndex", value.orderIndex);
    string memory finalJson = tojsonUint256(obj, "itemIndex", value.itemIndex);
    return vm.serializeString(objectKey, valueKey, finalJson);
  }
  ...
  function tojsonDynArrayDynArrayFulfillmentComponent(
    ...
    string memory out;
    for (uint256 i; i < length; i++) {
      out = tojsonDynArrayFulfillmentComponent(obj, vm.toString(i), value[i]);
    }
    return vm.serializeString(objectKey, valueKey, out);
  }
  ...
  function tojsonFuzzTestContext(
    ...
    tojsonAddress(obj, "conduitController", address(value.conduitController));
