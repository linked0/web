...
import {AdvancedOrderLib, FulfillAvailableHelper, MatchFulfillmentHelper, OrderComponentsLib, OrderLib, OrderParametersLib} from "seaport-sol/src/SeaportSol.sol";
...
import {FuzzTestContext, FuzzTestContextLib, FuzzParams} from "./FuzzTestContextLib.sol";
...
abstract contract FuzzExecutor is Test {
  ...
  function exec(FuzzTestContext memory context, bool logCalls) public {
    ...
    if (_action == context.seaport.fulfillOrder.selector) {
      ...
      context.returnValues.fulfilled = context.seaport.fulfillAdvancedOrder{
        value: context.executionState.value
      }(
        order,
      ...
      (bool[] memory availableOrders, Execution[] memory executions) = context
        .seaport
        .fulfillAvailableOrders{value: context.executionState.value}(
      ...
      (bool[] memory availableOrders, Execution[] memory executions) = context
        .seaport
        .fulfillAvailableAdvancedOrders{value: context.executionState.value}(
      ...
      for (uint256 i; i < orders.length; ++i) {
        ...
        orderComponents[i] = order.toOrder().parameters.toOrderComponents(
