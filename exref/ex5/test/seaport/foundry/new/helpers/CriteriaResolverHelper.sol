...
import {AdvancedOrder, ConsiderationItem, CriteriaResolver, OfferItem} from "seaport-sol/src/SeaportStructs.sol";
...
contract CriteriaResolverHelper {
  ...
  function wildcardIdentifierForGivenItemHash(
    ...
    if (!id.set) {
      revert("CriteriaResolverHelper: no wildcard set for given item hash");
    }
      ...
      for (uint256 j; j < order.parameters.offer.length; j++) {
        ...
        if (
          ...
          if (offerItem.identifierOrCriteria == 0) {
            bytes32 itemHash = keccak256(abi.encodePacked(i, j, Side.OFFER));
  ...
  function generateCriteriaMetadata(
    ...
    bytes32[] memory proof = MERKLE.getProof(leaves, selectedIdentifierIndex);
  ...
  function generateWildcard(
    ...
    bytes32 itemHash = keccak256(abi.encodePacked(orderIndex, itemIndex, side));
  ...
  function shiftWildcards(
    ...
    for (uint256 i = originalLength; i > insertionIndex; --i) {
      bytes32 itemHash = keccak256(abi.encodePacked(orderIndex, i - 1, side));
