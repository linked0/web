...
import {BalanceErrorMessages, ERC721TokenDump, ExpectedBalances} from "./helpers/ExpectedBalances.sol";
...
contract ExpectedBalancesTest is Test {
  ...
  function testAddTransfers() external {
    ...
    executions[2] = Execution({
      offerer: bob,
      conduitKey: bytes32(0),
      item: ReceivedItem(ItemType.ERC721, address(erc721), 1, 1, payable(alice))
    });
  ...
  function testNativeInsufficientBalance() external {
    vm.expectRevert(
      bytes(
        BalanceErrorMessages.insufficientNativeBalance(alice, bob, 0, 1, false)
      )
    );
    balances.addTransfer(
      Execution({
  ...
  function testNativeExtraBalance() external {
    ...
    vm.expectRevert(
      bytes(BalanceErrorMessages.nativeUnexpectedBalance(alice, 0, 0.5 ether))
    );
  ...
  function testNativeNotTransferred() external {
    ...
    vm.expectRevert(
      bytes(BalanceErrorMessages.nativeUnexpectedBalance(bob, 0.5 ether, 0))
    );
    balances.checkBalances();
  }
  ...
  function testERC20InsufficientBalance() external {
    ...
    balances.addTransfer(
      Execution({
        offerer: alice,
        conduitKey: bytes32(0),
        item: ReceivedItem(ItemType.ERC20, address(erc20), 0, 200, payable(bob))
      })
    );
  }
  ...
  function testERC20ExtraBalance() external {
    ...
    balances.addTransfer(
      Execution({
        offerer: alice,
        conduitKey: bytes32(0),
        item: ReceivedItem(ItemType.ERC20, address(erc20), 0, 5, payable(bob))
      })
    );
    vm.prank(alice);
    ...
    vm.expectRevert(
      bytes(
        BalanceErrorMessages.erc20UnexpectedBalance(address(erc20), alice, 5, 6)
      )
    );
    balances.checkBalances();
  }
  ...
  function testERC20MultipleSenders() external {
    ...
    balances.addTransfer(
      Execution({
        offerer: alice,
        conduitKey: bytes32(0),
        item: ReceivedItem(ItemType.ERC20, address(erc20), 0, 50, payable(bob))
      })
    );
    balances.addTransfer(
      Execution({
  ...
  function xtestERC721InsufficientBalance() external {
    ...
    balances.addTransfer(
      Execution({
        offerer: alice,
        conduitKey: bytes32(0),
        item: ReceivedItem(ItemType.ERC721, address(erc721), 1, 1, payable(bob))
      })
    );
  }
  ...
  function testERC721MultipleIdentifiers() external {
    ...
    balances.addTransfer(
      Execution({
        offerer: alice,
        conduitKey: bytes32(0),
        item: ReceivedItem(ItemType.ERC721, address(erc721), 2, 1, payable(bob))
      })
    );
    vm.prank(alice);
