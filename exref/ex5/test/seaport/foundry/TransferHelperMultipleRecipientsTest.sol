...
contract TransferHelperMultipleRecipientsTest is BaseOrderTest {
  ...
  function _deployStubTokens() internal {
    for (uint256 i; i < 3; i++) {
      ...
      vm.label(address(stub20), string.concat("StubERC20 #", i.toString()));
      vm.label(address(stub1155), string.concat("StubERC1155 #", i.toString()));
      vm.label(address(stub721), string.concat("StubERC721 #", i.toString()));
  ...
  function setUp() public override {
    ...
    vm.label(address(invalidERC721Receiver), "invalid (error) ERC721 receiver");
    ...
    if (
      ...
      for (uint256 i; i < itemsWithRecipient.length; i++) {
        ...
        for (uint256 j; j < firstRecipientTransfer.items.length; j++) {
          TransferHelperItem memory item = firstRecipientTransfer.items[j];
          ...
          if (item.itemType == ConduitItemType.ERC20) {
            ...
            emit Transfer(from, firstRecipientTransfer.recipient, item.amount);
          } else if (item.itemType == ConduitItemType.ERC721) {
  ...
  function _performMultiItemTransferAndCheckBalances(
    ...
    if (
      // Compare hashes as we cannot directly compare bytes memory with bytes storage.
      keccak256(expectRevertDataWithConduit) == keccak256(REVERT_DATA_NO_MSG)
    ) {
      vm.expectRevert();
      ...
      for (uint256 i; i < itemsWithRecipient.length; i++) {
        ...
        for (uint256 j; j < singleItemsWithRecipient.items.length; j++) {
          TransferHelperItem memory item = singleItemsWithRecipient.items[j];
          // expect all 3 indexed topics plus data since Transfer event has same signature for ERC20/ERC721,
          // but tokenId is indexed for 721 and not for ERC20 (so amount is data)
          // ERC1155 has three indexed topics plus data.
          if (item.itemType == ConduitItemType.ERC20) {
