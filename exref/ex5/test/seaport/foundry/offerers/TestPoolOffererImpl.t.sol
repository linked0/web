...
contract TestPoolOffererImpl is Test {
  ...
  function setUp() public {
    ...
    test = TestPoolImpl(
      factory.createPoolOfferer(address(erc721), tokenIds, address(erc20), 1e18)
    );
  }
  ...
  function testPreviewOrder() public {
    ...
    (SpentItem[] memory spentItems, ReceivedItem[] memory receivedItems) = test
      .previewOrder(
        address(0),
        address(this),
        minimumReceived,
  ...
  function testGenerateOrder() public {
    ...
    vm.prank(seaport);
    (SpentItem[] memory spentItems, ReceivedItem[] memory receivedItems) = test
      .generateOrder(address(this), minimumReceived, maximumSpent, "");
  ...
  function testGenerateOrder_wildcard1() public {
    ...
    assertTrue(spentItems[0].identifier != spentItems[1].identifier, "same id");
  }
