...
import {ConsiderationItem, OfferItem, Order, OrderComponents, OrderParameters} from "seaport-types/src/lib/ConsiderationStructs.sol";
...
contract FulfillOrderTest is BaseOrderTest {
  ...
  modifier validateInputs(FuzzInputsCommon memory args) {
    ...
    vm.assume(
      args.paymentAmts[0].add(args.paymentAmts[1]).add(args.paymentAmts[2]) <=
        uint128(MAX_INT)
    );
    _;
  }

  modifier validateInputsWithTip(FuzzInputsCommon memory args, uint256 tipAmt) {
    vm.assume(
      args.paymentAmts[0] > 0 &&
        args.paymentAmts[1] > 0 &&
        args.paymentAmts[2] > 0 &&
    ...
    vm.assume(
      args.paymentAmts[0].add(args.paymentAmts[1]).add(args.paymentAmts[2]).add(
        tipAmt
      ) <= uint128(MAX_INT)
    );
    _;
  ...
  function testNullAddressSpendReverts() public {
    ...
    test(this.nullAddressSpendReverts, Context(consideration, empty, 0, 0, 0));
  }

  function nullAddressSpendReverts(Context memory context) external stateless {
  ...
  function testFulfillOrderEthToErc721(
    ...
    test(this.fulfillOrderEthToErc721, Context(consideration, inputs, 0, 0, 0));
  }
  ...
  function fulfillOrder2098(Context memory context) external stateless {
    ...
    _configureOrderParameters(bob, address(0), bytes32(0), globalSalt++, false);
  ...
  function fulfillOrderEthToErc721(Context memory context) external stateless {
    bytes32 conduitKey = context.args.useConduit ? conduitKeyOne : bytes32(0);

    test721_1.mint(alice, context.args.id);
    offerItems.push(
      OfferItem(ItemType.ERC721, address(test721_1), context.args.id, 1, 1)
    );
  ...
  function fulfillOrderEthToErc1155(Context memory context) external stateless {
    bytes32 conduitKey = context.args.useConduit ? conduitKeyOne : bytes32(0);
