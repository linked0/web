...
import {AdvancedOrder, CriteriaResolver, Execution, Fulfillment, FulfillmentComponent, Order, ReceivedItem, SpentItem} from "../../../seaport-types/src/lib/ConsiderationStructs.sol";
  ...
  function processExplicitExecutionsFromAggregatedComponents(
    ...
    for (uint256 i; i < considerationComponents.length; i++) {
      ...
      for (uint256 j = 0; j < aggregatedComponents.length; j++) {
        ...
        if (
          component.itemIndex < considerationOrderDetails.consideration.length
        ) {
  ...
  function processExecutionFromFulfillment(
    ...
    if (aggregatedOfferAmount > aggregatedConsiderationAmount) {
      sourceOrder
        ...
        .amount += (aggregatedOfferAmount - aggregatedConsiderationAmount);
